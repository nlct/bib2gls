...
\item[\longargfmt{split-on-type}] Split the entries into separate
files according to the \field{type} field. Any entries that don't
have the \field{type} field set or have it set to \ics{glsdefaulttype}
will be written to \meta{bib file}. Otherwise entries will be
written to \metafilefmt{}{type}{.bib} (in the same directory as
\meta{bib file}) where \meta{type} is the value of the \field{type}
field. This option automatically implements \longargfmt{ignore-type}
and \longargfmt{no-overwrite}.

\item[\longargfmt{no-split-on-type}] Don't split the entries into separate
files (default).

\item[\longargfmt{preamble-only}] Only parse the document preamble.
