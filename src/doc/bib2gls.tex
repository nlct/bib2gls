...
\newcommand*{\bibglsothergroup}[3]{#3.#2}
\newcommand*{\bibglsothergrouptitle}[3]{#1}
...
\newlength\imagewidth
\newcommand*{\pdftwocol}[2]{%
  \renewcommand{\tabcolsep}{2pt}%
  \setlength{\imagewidth}{\dimexpr.5\textwidth-\tabcolsep}%
  \def\tabcontents{\begin{tabular}{@{}cc@{}}}%
  \count0=0\relax
  \loop
  \advance\count0 by 1\relax
  \eappto\tabcontents{\noexpand\frame{%
   \noexpand\includegraphics[page=\the\count0,width=\noexpand\imagewidth]{#1}}}%
  \ifodd\count0
    \appto\tabcontents{&}%
  \else
    \appto\tabcontents{\\}%
...
creating a large \iext{tex} file containing many definitions that
you frequently use in documents. This file can then simply be
loaded using \ics{input} or \ics{loadglsentries}, but a large file
...
\verb|\| or a tilde \verb|~| or a dollar symbol \verb|$| or braces
\verb|{| \verb|}| (and when the \csopt{sort} option is not
...
\code{\%} is best avoided in field values, the subscript and
superscript characters \code{\_} and \code{\textasciicircum} should
either be encapsulated by \code{\$} or by \cs{ensuremath}, which
will be picked up by the check for \code{\$} or
\code{\glsbackslash}, and the other special characters would
...
\item if the \field{parent} or \field{alias} field contain
\verb|\| or \verb|{| or \verb|}| the interpreter is used 
since these fields must be just a label referencing another entry
(other special characters aren't checked as they won't expand
...
files. Note that the \field{see} and \field{seealso} fields
don't use the interpreter, but \bibgls\ will try to fully
expand the \field{seealso} field using the parser library
(since it should only contain a comma-separated list of labels).
The parser has a different concept of expansion to \LaTeX\
and will expand some things that aren't expanded by \LaTeX\
(such as \cs{MakeUppercase}) and won't expand other commands
that would be expanded by \LaTeX\ (such as commands defined
in terms of complicated internals).
The \field{see} field may legitimately contain \LaTeX\ code
in the optional tag part (such as \cs{seealsoname}) so it's not
...
\isty{pifonts}, \isty{wasysym}, \isty{amssymb}, \isty{stix}, \isty{mhchem}
and \isty{bpchem}. There's also partial support for \isty{siunitx}'s
\ics{si} command.}\ or any custom
...
The interpreter treats a tilde character \verb|~| as a non-breakable
space (default).

\argsection{break-space}

The interpreter treats a tilde character \verb|~| as a normal
space.
...
\ics{printunsrtglossary} if group headings are required (for
...
than using \ics{input} or \ics{loadglsentries}, which requires all the
...
actually be using
\begin{alltt}
\ics{glslink}\oarg{\meta{def-options},\meta{options}}\margm{label}\marg{\csfmt{}\meta{csname}\margm{text}}
\end{alltt}
...
manual~\cite{glossaries-extra} for further details.)
...
interpreter is on) are carried over to the next resource set.  The
\sty{glossaries-extra} package doesn't parse the options, but just
writes the information to the \ext{aux} file. This means that any
invalid options will be reported by \bibgls\ not by
...
\gls{glsxtrresourcefile} inputs the \ext{glstex} file.
...
be set using \ics{glssetwidest} provided by the \styfmt{glossaries}
...
preamble, this option may not work. For example, if one entry has
the \field{name} defined as
\begin{verbatim}
...
using:
\begin{alltt}
\ics{glssetwidest}\oargm{level}\marg{\ics{glsentryname}\margm{id}}
\end{alltt}
where \meta{id} is the entry's label. This leaves \TeX\ to
compute the width according to the document fonts.

If \csopt{type} has been set, the \ics{glssetwidest} command
...
Rather than directly using \ics{glssetwidest}, \bibgls\ provides
\csref{bibglssetwidest} (where the type isn't known) and 
\gls{bibglssetwidestfortype} (where \ics{glssetwidest} needs to be
...
Pattern class}~\cite{pattern}. The pattern is anchored (\code{oo.*} matches
...
only if the target field isn't already set. This action is
performed after \csopt{ignore-fields}.
...
\code{\$\meta{maths}\$}, \ics{ensuremath}\margm{maths} 
...
can be used to append a full stop to the end of all the
descriptions. This can be awkward if some of the descriptions
end with punctuation characters. This resource option can be
used instead. The \meta{value} may be one of:
\begin{itemize}
\item\optfmt{none}: don't append a full stop (default);
\item\optfmt{all}: append a full stop to all \field{description}
  fields in this resource set;
\item\optfmt{check}: selectively append a full stop (see below).
\end{itemize}
Note that if you have dual entries and you use this option to
append a full stop, then it will be copied over to the mapped field.
This is different to the \styopt{postdot} option which doesn't
add the dot to the field but incorporates it in the post-description
hook.
...
This option is always performed before \cs{post-description-dot}
...
description should not contain both commands.
...
an empty group not with \cs{nopostdesc}. It also won't be
stripped from
\begin{verbatim}
description={sample{\nopostdesc}}
\end{verbatim}
because the grouping hides it.
...
If \meta{value} omitted, \csopt[\ics{at}.]{loc-suffix} is assumed.
...
class~\cite{rulebasedcollator}. Remember that the options will be expanded as they
are written to the \iext{aux} file, so be careful of
...
\ics{glsbackslash} and \ics{glstildechar} which can be
used to produce a literal backslash (\verb|\|) and tilde (\verb|~|).
...
the values from the given \meta{field} (without using the
interpreter).
\end{itemize}
In each case (other than \csopt[none]{identical-sort-action}) a
simple string comparison is used. If \meta{value} isn't a recognised
keyword or valid field an error will occur.
The default setting is \csopt[id]{identical-sort-action}.
If you want to use a field that's not a simple label (such as the
\field{category} field) you may find it better to use
\csopt{sort-suffix} for locale-sensitive sorts.
...
\verb|#|, you will need to precede it with \ics{cs.string} (for example
...
class~\cite{decimalformat}. You can use \stringu\meta{hex} to indicate Unicode characters
...
(rather than using commands like \cs{glsentryname}) allows it 
...
\csopt{loc-suffix}. For example, with \csopt[\ics{at}.]{loc-suffix}
...
\styfmt{glossaries-extra} and \bibgls\ if you want to try these out.
If you get any undefined control sequence or undefined style errors
then you need to update your \TeX\ distribution.
Use the \longarg{group} switch when invoking \bibgls\ for all these
examples if you want the glossaries divided into groups.
The set of system calls for the document build in the examples below
may require an extra \LaTeX\ run to ensure the PDF bookmarks are
up-to-date when \sty{hyperref} is used.
...
\qt{the}). Although it's functionally the same as \gls{sortname},
it's semantically different and allows for adjustments for one type
of entry (such as a person's name) without affecting the other (such as a
book or film title).
...
Each \field{name} field uses \ics{ce} provided by \sty{mhchem}
...
All entries are defined using \atentry{entry}. With this entry
type, if the \field{sort} field is omitted \bibgls\ will fallback on
the \field{name}. This will require the use of the interpreter as
the names all contain a command but \bibgls\ recognises the
\sty{mhchem} package and has a limited ability to interpret \cs{ce}.
If \atentry{entry} is changed to \atentry{symbol} the fallback for
the \field{sort} will be the entry's label.

The entries all have a custom field \fieldfmt{identifier} set to 
\code{chemical}. This will be ignored by \bibgls\ unless it's
...
The \exfile{baseunits.bib} file contains base SI units. The entries
are all defined using \atentry{entry} with a \field{name},
\field{symbol} and \field{description} field. The \field{symbol}
fields all use \ics{si} provided by the \sty{siunitx} package, so
that package must be loaded in the document. This is one of the
small number of packages recognised by \bibgls, so it's possible to
sort according to the symbol if required.
...
\end{verbatim}
(where \ics{DTMdisplaydate} is provided by the \sty{datetime2}
...
aliased with \csopt{field-aliases}.
(Remember that \bibgls\ isn't \appfmt{bibtex} and doesn't recognise
\appfmt{bibtex}'s fields or name syntax.)
...
\exfile{interpret-preamble.bib}.
The entries all have a custom field \fieldfmt{identifier} set to 
\code{film} and other custom fields \fieldfmt{director} and
...
loaded in separate resource sets with different prefixes).
...
you could have a \fieldfmt{cast} field with a comma-separated list
of actor names. Similarly for the \fieldfmt{director} field. Some of
the films actually had more than one director but only one is listed
per film in this sample file for simplicity. The book on which the film is based
...
result as the inserted full stop immediately follows the question
mark. This is an awkward situation. One possibility is to explicitly 
put the full stop at the end of the \field{description} field for all the 
other entries and omit it for the problematic entries. 
...
\end{verbatim}
(This was only introduced to \styfmt{glossaries-extra} v1.22.)
This doesn't interfere with the \idx{postdescriptionhook} but if a hook
is provided the problem will likely go away (unless the hook also
ends with a punctuation character).

An alternative approach is to use
\csopt[check]{post-description-dot}. Unfortunately in this case 
the question mark is hidden inside the \cs{gls} command.
This could be fixed by using a different category for entries
with names that end with sentence-terminating punctuation. Then the
\catattr{discardperiod} attribute can be set for that category. When
combined with \csopt{post-description-dot} this would allow
the \idx{postlinkhook} to look ahead and discard the dot.

Since \csopt{strip-trailing-nopost} can be used
to strip \cs{nopostdesc} and \cs{glsxtrnopostpunc} from the end
of the \field{description} field, the simplest option is
to put one of those commands at the end of the field 
and strip it when it's not needed. I've used \cs{nopostdesc} as it's
the older of the two commands and it will have to be stripped anyway
if the \idx{postdescriptionhook} is used.
...
efficient to instruct \bibgls\ to not save the number list with the
\csopt[false]{save-locations} resource option.
...
shown in \figureref{fig:sample-chemical.pdf}.
...
the \catattr{glossnamefont} attribute and the font used by commands like
...
field and, since the \abbrstyle{long-only-short-only} style sets
...
abbreviations and this might not be appropriate for them. Another
...
title (\qt{Derived Units} in this case). Since there's no
\csopt{type} set, all entries end up in the main glossary, but since
there are two resource commands the glossary ends up with sorted blocks.
...
sorted blocks this example has two glossaries. I've used the
\styopt{section} package option to use \ics{section*} for the
...
\label{fig:sample-units2.pdf}
\end{figure}

\filesection{sample-units3.tex}

This is another example that uses the \exfile{baseunits.bib} and
\exfile{derivedunits.bib} files. This time I want two glossaries
containing all the units (base and derived) where the first glossary
is ordered by name and the second is ordered by symbol. This can be
done with a single resource command that instructs \bibgls\ to make
\atentry{entry} behave like \atentry{dualsymbol}. This causes the
\field{name} and \field{symbol} fields to be swapped in the dual
list. The fallback for the \field{sort} field is now the entry label
so I need \csopt[name]{sort-field} to ensure that the sorting is
by the \field{name} field instead.
...
limited) support for \sty{siunitx} and is able to interpret
the \ics{si} commands in the sample \ext{bib} files. Since 
SI units are a mix of letters and numbers I've used one of the
...
with \csopt{type} and \csopt{dual-type}:
\begin{verbatim}
type=main,dual-type=units
\end{verbatim}
The complete document code is listed below. The document build is:
\begin{verbatim}
...
Once possible solution would be to also assign prefixes for the book
...
\exfile{films.bib}) or alias the custom \fieldfmt{identifier} format to
\field{category}. I've chosen the later method and also provided
an alias for the custom \fieldfmt{year} field:
\begin{verbatim}
...
identical. If a letter comparison was used instead, they would no
longer be considered identical, but in this case the film would
...
birth and an index of all the people without their details and with
a number list indicating where the people were mentioned in the
...
\idx{postlinkhook} \csfmt{glsxtrpostlink\meta{category}}. If the
\field{category} field isn't specified it will default to
\code{general} (for entries defined with \atentry{entry}), so I
could just redefine \csfmt{glsxtrpostlinkgeneral} but to allow for
...
\csfmt{glsxtrpostname\meta{category}}, in this case
...
The \idx{postdescriptionhook} can be used to appended the birth and
...
The first two glossaries would look better with a terminating full
stop, so I've used the \styopt{postdot} package option. This can
locally be switched off for the index with \code{nopostdot}.
...
\usepackage[record,% using bib2gls
 ...
 stylemods={list,bookindex},% load glossary-bookindex.sty and fix styles
...
\begin{verbatim}
\printunsrtglossary*
 [type=index,style=bookindex,nopostdot]
 {%
   \let\sortname\swaptwo
   \let\sortart\swaptwo
   \let\sortvonname\swapthree
...
\verb|\| or \verb|{| or \verb|}|. This means that with this field
alias, \bibgls\ will attempt to interpret the field contents. So all
that's needed is to ensure that \bibgls\ is given a definition of
\gls{sortmediacreator} that ignores the first argument and converts
the second argument to lower case. This definition is available in
