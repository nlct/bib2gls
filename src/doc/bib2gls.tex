...
\idxprefix{extn} will be replaced by the corresponding
...
are all defined using \atentry{symbol} with a \field{name},
...
sort according to the symbol if required. Remember that
\atentry{symbol} entry types use the label as the fallback field for
\field{sort}. In this case the labels all happen to match the
\field{name}.
...
languages, vegetables, minerals, animals, chemical formula) and also an
...
\exfile{chemicalformula.bib} file):
\begin{codeenv}
\csopt[
  identifier=category,
  formula=symbol,
