...
\atentry{symbol}. This example file requires the \sty{stix} package
...
a small version. However, the interpreter works well enough to guess
...
approaches are to sort according to document use (\cs[use]{sort})
or sort according to the \field{description} field.
...
This example uses the files \exfile{bigmathsymbols.bib}
and \exfile{mathgreek.bib}. The \sty{stix} package is required for
some of the commands used in \exfile{bigmathsymbols.bib}, so that
must be loaded in the document.

I decided to use the \glostyle{mcolalttree} style for this document,
...
\end{verbatim}
Since I'm not using a group-based style I don't need the
\longarg{group} switch. I'm also not referencing any of the entries
in the document as I'm just generating a complete list of all the
defined symbols. This means I need to tell \bibgls\ to select all
entries and don't bother saving the \field{location} field:
\begin{verbatim}
