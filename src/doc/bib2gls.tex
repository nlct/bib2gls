...
indicate something too complex for the interpreter to handle.} The
other case is with \csopt{set-widest} when determining the width of
the \field{name} field. The \longarg{no-interpret} switch will turn
off this function, but the library will still be used to parse the
\ext{aux} and \ext{bib} files.
...
\ics{nopostdesc} or \ics{nopostpunc}, then a dot isn't appended.
...
appended (unless it's \cs{nopostpunc} or \cs{nopostdesc}) even if
...
\optfmt{true} any trailing ungrouped \ics{nopostdesc} or \ics{nopostpunc}
...
\exfile{interpret-preamble.bib} file can then be loaded to provide
alternative definitions for \bibgls's interpreter.
...
This provides definitions of \gls{sortname}, \gls{sortvonname} and
\gls{sortart} in \atentry{preamble} that can be picked up by the interpreter
and used during sorting.
...
All entries are defined using the \atentry{entry}. With this entry
...
One entry (\code{caesar}) has a \field{first} field. In that case
the \field{first} field is slightly different from the \field{name}
field (\qt{Gaius} is omitted). The other entries don't have a
...
(or \qt{log line}). Use this as a starting point if you need data
...
A third possibility is to use \ics{nopostpunc} instead:
\begin{verbatim}
description={a film starring Harrison Ford, Rutger Hauer
 and Sean Young loosely based on the novel
 \gls{ext1.doandroidsdreamofelectricsheep}\nopostpunc},
...
Since the \csopt{strip-trailing-nopost} option can be used
to strip \cs{nopostdesc} and \cs{nopostpunc} from the end
...
The document doesn't include any indexing commands, such as \cs{gls}, so
I've use \csopt{selection} to select all entries in the \ext{bib}
...
(\code{film.thebigsleep} comes before \code{film.thebigsleep}) and
...
death dates. Although all the entries in \exfile{people.bib} have a 
\fieldfmt{died} field, I've added a check for the corresponding
\field{user3} field in case new entries are added for people who are
still alive:
\begin{verbatim}
