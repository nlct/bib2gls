...
\begin{verbatim}
@symbol{sun,
  name={\textweathersymbol{16}},
  description={sunny},
  identifier={weather}
}

@symbol{lightning,
  name={\textweathersymbol{26}},
...
\code{weather}, respectively.

Neither of these packages are recognised by \bibgls, which means
that \csopt{set-widest} won't work with this data nor is it suitable
for sorting according to the \field{name} field. Instead, either
sort by label (which is the default for \atentry{symbol}) or
by the \field{description}. If you want to use one of the \glostyle{alttree}
styles you'll need to use \ics{glsFindWidestTopLevelName} to
...
indexed, so I've omitted the \csref{selection} option I used in the
...
Unfortunately, since \bibgls\ doesn't recognise \sty{ifsym} or
\sty{marvosym}, there's no point using \csopt{set-widest}. Instead I
have to go back to using the non-\bibgls\ method:
\begin{alltt}
\ics{glsFindWidestTopLevelName}
\end{alltt}
