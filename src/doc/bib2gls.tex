...
the document using
\begin{definition}\labelcs{glsxtrresourcefile}
...
\subsection{\csoptnv{interpret-preamble}=\margm{boolean}}
...
\end{alltt}
instead of using \cs{bibglsflattenedchildpostsort}. This defaults to
just \meta{name}.
...
the name is again considered a homograph.)
...
\end{verbatim}
The child entries don't have a \field{name} field, so it's assumed
to be the same as the parent field. Here's an example document where
both child entries are used:
\begin{verbatim}
\documentclass{article}
...
set to
\begin{verbatim}
\bibglsflattenedhomograph{glossary}{glossary}
\end{verbatim}
(where the first argument is the name and the second argument is the
label of the parent entry, which just happens to be the same in this
case).

This means that the name simply appears as \qt{glossary}, even if
the \optref[postsort]{flatten-lonely} option is used.
