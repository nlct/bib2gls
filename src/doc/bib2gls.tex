...
\optref{letter-number-rule} is used to determine the result. 
(The \optref{dual-sort} and \optref{secondary} options are governed
by the corresponding \optref{dual-letter-number-rule}, and
\optref{secondary-letter-number-rule}.)
...
Where two non-numbers are compared, these methods behave like their
\optfmt{letter-}\emph{modifier} counterparts.
\begin{itemize}
...
comparison between a number and non-number. The \meta{value} may be
one of:
\begin{itemize}
\item\optfmt{before lower}: numbers are considered less than lower
case characters (default), which means that numbers are also considered greater
than all other characters including upper case and punctuation;
\item\optfmt{before upper}: numbers are considered less than upper
case characters, which means that numbers are also considered greater
than all other characters including lower case and punctuation;
\item\optfmt{before letter}: numbers are considered less than any
letter (regardless of case), which means that numbers are also
considered greater than all non-letter characters;
\item\optfmt{after letter}: numbers are considered greater than any
letter (regardless of case), which means that numbers are also
considered less than all non-letter characters;
...
Use \optref{dual-letter-number-rule} for \optref{dual-sort} and
\optref{secondary-letter-number-rule} for \optref{secondary}.
