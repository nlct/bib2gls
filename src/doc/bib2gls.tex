...
Collapse any explicit location range into a normal record if the locations
are the same (default).

\argsection{no-collapse-same-location-range}

Don't collapse any explicit location range into a normal record if the locations
are the same.
...
It's possible that you may not want to lose certain location
formats, even if it means having duplicate locations. For example,
if you want to move a primary location using
\csopt[remove]{save-primary-locations}. In which case, use this
...
coincident start and end locations. If the primary record is between
the start and end format markers then the range can't collapse
to an ordinary record.
...
If you need a field to store the dual description in (and you're not
simply swapping known fields around), then you can use the special
\field{dualdescription} field and add it to your map.
...
\idxpl{resourceset} with \csopt[selected before]{selection}.
...
This setting won't add any \field{see} or \field{seealso} lists to
the \idx{locationlist}. The given field will be set, so you can
access the information, but there's no guarantee that the
cross-referenced entry will have been selected. The \field{alias}
cross-reference will be added to the \idx{locationlist} but you will
need to ensure that the target is also selected (or use
\csopt[omit]{alias} to suppress it).
...
haven't been referenced in the document. The above notes regarding
the cross-reference lists also applies.
...
selected in a previous \idx{resourceset}. This is intended
...
inserted when there's no entry in the current \idx{resourceset} that
...
This setting will save the definition index that's used by
...
Note that if you want to order all entries by definition, it's
better to use \csopt[none]{sort}, which doesn't perform any sorting,
so the order will be by definition.
...
This setting will save the order of use index that's used by
...
of use corresponds to the first time the entry is recorded (indexed
and not ignored or merged) in the document.

Note that if you want to order all entries by use, it's
better to use \csopt[use]{sort}, which doesn't perform any sorting.
...
\idx{locationlist}. You may want to consider using the
\longswitch{retain-formats} switch with this setting if you don't
want to lose a partial location match (for example, if the primary
location coincides with the start of an explicit range).
\end{itemize}
...
Similar to \csopt{match} but determines whether or not to include 
a primary entry in the secondary list. The syntax is the same but
this option is governed by \csopt{secondary-match-op} and
\csopt{secondary-match-action}. Note that if an entry hasn't been
selected for the primary list then it won't be added to the
secondary list, regardless of this setting.

\optsection{secondary-not-match}

Similar to \csopt{not-match} but determines whether or not to include 
a primary entry in the secondary list. The syntax is the same but
this option is governed by \csopt{secondary-match-op} and
\csopt{secondary-match-action}. Note that if an entry hasn't been
selected for the primary list then it won't be added to the
...
As \csopt{match-op} but for the secondary list selection.

\optsection{secondary-match-action}

As \csopt{match-action} but for the secondary list selection.
...
definition index of the entry identified by \meta{label}. This
...
order of use index of the entry identified by \meta{label}. This
command will only be provided in the \ext{glstex} file if 
