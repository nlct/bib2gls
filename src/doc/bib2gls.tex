...
If you use the \sty{glossaries-prefix} package, the \field{prefix}
set of fields become available. The default behaviour of \ics{pgls} is for no
...
where an non-breakable space should be used. It's a bit tiresome
having to remember to put \ics{space} or \idx{nbspchar} at the end
of the prefix value. This option allows the automatic insertion of a
space.
...
\item \optfmt{space}: append a space to the prefix field unless an
exception applies (see below);

\item \optfmt{space or nbsp}: append \cs{space} or \idx{nbspchar} to the prefix field unless an
exception applies (see below). The non-breakable space is used if
the field value consists of a single character.

\end{itemize}

The exceptions are when the field value ends with a character
identified by \csopt{append-prefix-field-exceptions}.

\optsection[\subsubsection]{append-prefix-field-exceptions}

The value should be a sequence of characters. Spaces are stripped
from \meta{sequence} so \csopt[ ' - ]{append-prefix-field-exceptions}
is equivalent to \csopt['-]{append-prefix-field-exceptions}.

The default set is the straight apostrophe character (\hex{0027}),
the hyphen-minus character (\hex{002D}), the hyphen character
(\hex{2010}), the non-breaking hyphen (\hex{2011}), and the right
single quotation mark (\hex{2019}).
...
If you have used explicit text instead of \cs{seealsoname} then
consider performing a global search and replace on your file using
your text editor.
