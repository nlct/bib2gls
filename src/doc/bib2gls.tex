...
% arara: pdflatex
\documentclass[titlepage=false,indextotoc]{scrreprt}
...
If you don't require an indexing application, just use
\styopt{record} (or \styopt[only]{record}) and don't use \cs{makeglossaries}. The 
glossaries can be displayed using \cs{printunsrtglossary}
(or \cs{printunsrtglossaries}). (The \texttt{unsrt} part
of the command name indicates that the entries are listed in the
order of definition from \sty{glossaries-extra}'s point of view.
\bibgls\ determines this order of definition according to
the options supplied in \hyperref[sec:resourceopts]{\cs{glsxtrresourcefile}}.)
...
matches (see \sectionref{sec:locationopts}). For example, if
\texttt{emph} should override \texttt{textbf}, then
use
\begin{verbatim}
bib2gls --map-format "hyperbf:hypersf" mydoc
\end{verbatim}
...
\begin{verbatim}
bib2gls --map-format "emph:hyperbf,hypersf:hyperbf" mydoc
\end{verbatim}
Then \bibgls\ will process these records as follows:
\begin{enumerate}
\item Accept the first record (with \texttt{emph}).
...
\begin{itemize}
\item The \texttt{hypersf} format is mapped to \texttt{hyperbf},
...
\item The \texttt{emph} format is mapped to \texttt{hyperbf},
...
the new record is discarded with a warning.
\end{itemize}
...
\subsection{\csopt{charset}=\margm{encoding-name}}
\labelopt{charset}
...
\subsection{\csopt{src}=\margm{list}}
\labelopt{src}
...
\subsection{\csopt{selection}=\margm{value}}
\labelopt{selection}
...
\styfmt[warn]{undefaction} option, which means that you'll only get
...
\subsection{\csopt{ignore-fields}=\margm{list}}
...
\subsection{\csopt{category}=\margm{value}}
\labelopt{category}
...
\subsection{\csopt{type}=\margm{value}}
...
\csopt[\marg{same as entry}]{value} indicates that the entry
type should be used. There is no \meta{value} analogous to
\csopt[\marg{same as type}]{category}.
...
\subsection{\csopt{label-prefix}=\margm{tag}}
...
@index{goose,plural="geese"}
\end{verbatim}
Then if this \ext{bib} file is loaded with \csopt[\marg{gls.}]{label-prefix}
...
\subsection{\csopt{ext-prefixes}=\margm{list}}
...
\end{alltt}
for cross-references supplied by the \field{see} field and
\begin{alltt}
\cs{glsnoidxdisplayloc}\margm{prefix}\margm{counter}\margm{format}\margm{location}
\end{alltt}
for locations. You can iterate through the \field{loclist} value
...
\end{alltt}
Exactly duplicates are discarded. For example, if \texttt{cat}
...
\item Otherwise the duplicate record will be added to the
location list with a warning.
\end{itemize}
...
\subsection{\csopt{min-loc-range}=\margm{value}}
...
\margm{prefix-2}\margm{counter-2}\margm{format-2}\margm{location-2} follows
another location
\margm{prefix-1}\margm{counter-1}\margm{format-1}\margm{location-1} 
according to the following:
\begin{itemize}
\item \meta{prefix-1} is not equal to \meta{prefix-2} or
...
\item If either \meta{location-1} or \meta{location-2} are empty,
then the locations aren't considered consecutive.
\item If both \meta{location-1} and \meta{location-2} match the
pattern
\begin{verbatim}
...
then:
  ...
  \item if argument of the control sequence (group 3) is the same for
...
then:
  \begin{itemize}
  \item if group 3 of both pattern matches are equal the test is
...
then the locations aren't considered consecutive;
   \item If \meta{group-3} of the second pattern match is one more
than \meta{group-3} of the first pattern match, then the locations
are consecutive otherwise they're not consecutive.
  \end{itemize}
\item The next pattern matches for \meta{prefix}\meta{sep}\meta{n}
...
in the same way as the above decimal test.
\item The next pattern matches for \meta{prefix}\meta{sep}\meta{c}
...
performed in the same way as the decimal pattern above.
\item If none of the above, the locations aren't considered
consecutive.
\end{itemize} 

\subsection{\csopt{suffixF}=\margm{value}}
\labelopt{suffixF}
...
\subsection{\csopt{suffixFF}=\margm{value}}
\labelopt{suffixFF}
...
\subsection{\csopt{see}=\margm{value}}
\labelopt{see}
...
\subsection{\csopt{loc-prefix}=\margm{value}}
\labelopt{loc-prefix}
...
\item \optfmt{list}: equivalent to \csopt[\marg{\cs{pagelistname} }]{loc-prefix}.
\item \optfmt{true}: equivalent to 
\csopt[\margm{page},\margm{pages}]{loc-prefix}, where \meta{page} and
...
\subsection{\csopt{sort}=\margm{value}}
\labelopt{sort}
...
\subsection{\csopt{sort-field}=\margm{label}}
\labelopt{sort-field}
...
\subsection{\csopt{dual-sort}=\margm{value}}
\labelopt{dual-sort}
...
\subsection{\csopt{dual-sort-field}=\margm{value}}
...
\subsection{\csopt{dual-prefix}=\margm{value}}
...
\subsection{\csopt{dual-type}=\margm{value}}
\labelopt{dual-type}
...
\subsection{\csopt{dual-category}=\margm{value}}
...
\subsection{\csopt{dual-entry-map}=\marg{\margm{list1},\margm{list2}}}
...
\subsection{\csopt{dual-abbrv-map}=\marg{\margm{list1},\margm{list2}}}
...
\subsection{\csopt{dual-symbol-map}=\marg{\margm{list1},\margm{list2}}}
...
\subsection{\csopt{dual-entry-backlink}=\margm{boolean}}
...
\subsection{\csopt{dual-abbrv-backlink}=\margm{value}}
...
\subsection{\csopt{dual-symbol-backlink}=\margm{value}}
\labelopt{dual-symbol-backlink}

This is analogous to \csopt{dual-entry-backlink} but for entries
defined with \atentry{dualsymbol} instead of \atentry{dualentry}.

\subsection{\csopt{dual-backlink}=\margm{value}}
\labelopt{dual-backlink}

Shortcut for \csopt[\margm{value}]{dual-entry-backlink}, 
\csopt[\margm{value}]{dual-abbrv-backlink}, and
\csopt[\margm{value}]{dual-symbol-backlink}.

\subsection{\csopt{dual-field}=\margm{value}}
\labelopt{dual-field}
