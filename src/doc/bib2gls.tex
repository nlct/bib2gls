...
first letter of all the sort values within that group (used with
alphabetical, letter or letter-number sort methods, such as \csopt[locale]{sort}
or \csopt[letternumber-nocase]{sort}).
...
\item \idxpl{emptygroup} where the sort value is empty. This will
occur when the sort value consists solely of commands that \bibgls\
can't interpret for the alphabetical, letter or letter-number sort
methods. The group title is set with \csref{bibglsemptygroup}.
...
\item \idxpl{datetimegroup} for all entries sorted by a date-time
comparison (such as \csopt[datetime]{sort}). 
The group title is set with \csref{bibglsdatetimegroup}.

\item \idxpl{dategroup} for all entries sorted by a date
comparison (such as \csopt[date]{sort}). 
The group title is set with \csref{bibglsdategroup}.

\item \idxpl{timegroup} for all entries sorted by a date
comparison (such as \csopt[time]{sort}). 
The group title is set with \csref{bibglstimegroup}.

\item \idxpl{customgroup} for all entries that have had the
group label explicitly set using the \csopt[\meta{label}]{group} 
resource option. You will need to use \ics{glsxtrsetgrouptitle}
to provide an associated title if the \meta{label} isn't the same as
the title. Remember that the label can't contain any active
characters, so you can't use non-ASCII characters in \meta{label}
with \sty{inputenc} (but you can with \sty{fontspec}).
\end{itemize}
...
transcript file (in this example, $8+1=9$).
