...
There's one entry that doesn't have the custom
\fieldfmt{value} field as there's no real number value.
...
This example just uses the \exfile{constants.bib} file. The aim here
...
\atentry{symbol} for semantic reasons:
\begin{codeenv}
\csopt[constant=symbol]{entry-type-aliases}
\end{codeenv}
...
\field{description} field and the custom \fieldfmt{definition} field to the
\field{user2} field. This means that by default the glossary list
...
\field{userii} with \idx{glsxtrifhasfield}.)
