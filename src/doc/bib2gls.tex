...
\item \optfmt{uc}: convert to \idx{uppercase} (using similar rules
as for \optfmt{firstuc}, described below, but continuing onto the
next token instead of stopping);\footnote{\ics{glsentrytitlecase}
will be converted to \ics{GLSxtrusefield}, which requires
\sty{glossaries-extra} v1.37+.}
\item \optfmt{lc}: convert to \idx{lowercase} (using similar rules
to \optfmt{uc} regarding non-letter content, with
\ics{glsentrytitlecase} converted to \ics{glsxtrusefield});
...
group) to \idx{uppercase}. This uses the following rules when trying
to convert the first \qt{letter} of \meta{content} to \idx{uppercase}:
\begin{enumerate}
\item if \meta{content} starts with \ics{protect} then skip and move on to the 
next token;

\item if \meta{content} starts with 
\code{\idx{mshiftchar}\meta{maths}\idx{mshiftchar}} or 
\code{\ics{NoCaseChange}\margm{text}} or
\code{\ics{ensuremath}\margm{maths}} or \code{\ics{si}\margm{text}}
or \code{\ics{glsentrytitlecase}\margm{text}} or any command in the form
\code{\csfmt{\ldots ref}\margm{label}} (such as \ics{ref} or
\ics{pageref}) then stop (no case change);\footnote{There was a bug in earlier
versions of \bibgls\ that skipped rather than stopped.}

\item if \meta{content} starts with any other control sequence
\csfmt{}\meta{csname} then:

\begin{enumerate}
\item if \csfmt{}\meta{csname} is known to have a case-variant, then
perform the appropriate substitution and stop (for example, 
\ics{glsentrytext} will be converted to \ics{Glsentrytext} and
\ics{ae} will be converted to \ics{AE});

\item if \csfmt{}\meta{csname} is \ics{glslink}, \ics{glsdisp} or 
\ics{glshyperlink}, perform the case-change on the relevant
argument (which becomes the new \meta{content});\footnote{In
general, it's best to avoid these commands in the \field{short}
field, but the same algorithm is used for
\csopt{description-case-change} where these commands may
legitimately occur.}

\item if \csfmt{}\meta{csname}\margm{text} then apply the case change to
\meta{text} (which becomes the new \meta{content}) and stop;

\item if \csfmt{}\meta{csname} isn't followed by a group, then stop 
(no case change applied);

\item if \margm{text} (a group) then convert the entire contents of
\meta{text} to \idx{uppercase} (using the same rules as for
\csopt[uc]{short-case-change}) and stop;
\end{enumerate}
...
\begin{codeenv}
\cs{MakeTextUppercase}\marg{html}
\end{codeenv}
...
the value may later be interpreted during sorting). When a command is 
encountered during the case-changing, it's checked against a list of 
known command names or specific patterns (such as \csfmt{glsentry}\meta{field},
where \meta{field} is a recognised field).

For example, suppose an abbreviation is defined using:
\begin{codeenv}
\field{short} = "z\ics{ae}\ics{oe}",
\end{codeenv}
...
With \csopt[uc-cs]{short-case-change}, the \field{short}
