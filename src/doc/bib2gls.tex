...
explicitly set the title in the optional argument. They still can't
be used in \cs{printunsrtglossaries}, since they're not included in
the list that this command iterates over.
...
The \exfile{no-interpret-preamble.bib} file contains a command
definition (\gls{sortop}) that's required by the document 
but needs to be hidden
from \bibgls's interpreter. This doesn't contain any entry
definitions and must be loaded first with
\csopt[false]{interpret-preamble}. (See the \csopt{sort} option on
page~\glsxtrpageref{opt.sort} for further details.)

\verbatiminput{../examples/no-interpret-preamble.bib}

There's no encoding line here as it's a trivial ASCII file.

\filesection{people.bib}

The \exfile{people.bib} file contains details about people. This
starts with a definition of \gls{sortop} in \atentry{preamble} that
can be picked up by the interpreter. This swaps the order of the
arguments (and inserts a comma) which allows \bibgls\ to sort by
surname. Remember that if \exfile{no-interpret-preamble.bib} is loaded
first, that definition will be the one in use in the document.
...
\field{first} with the \csopt{replicate-fields} option.

There's one name with a \qt{von} part. In this case the \field{name}
field is set to
\begin{verbatim}
\sortop{Manfred}{von Richthofen}
\end{verbatim}
which will come under the \qt{V} letter group. If you prefer that
this should come under \qt{R} instead, then you need to move the
\qt{von} part into the first argument:
\begin{verbatim}
\sortop{Manfred von}{Richthofen}
\end{verbatim}
Alternatively, you could provide a similar command with three
arguments for names with a \qt{von} part to allow for greater
flexibility.
