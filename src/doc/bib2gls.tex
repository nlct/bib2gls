...
\section{\longarg{packages} \meta{list} (or \shortarg{p} \meta{list})}
\labelarg{packages}
...
\section{\longarg{no-expand-fields}}
\labelarg{no-expand-fields}

By default, \cs{newglossaryentry} expands field values
...
\cs{glsnoexpandfields} to the \ext{glstex} file, which
...
\section{\longarg{expand-fields}}
\labelarg{expand-fields}

Don't write \cs{glsnoexpandfields} to the \ext{glstex} file,
...
expansion switched on with \cs{glssetexpandfield}\margm{field}.
...
\medskip
\centering
\begin{tabular}{l>{\raggedright}p{.75\textwidth}}
\bfseries Field & \bfseries Description\tabularnewline
\fielddef{duallong} & The long form of a dual
abbreviation mapped by \atentryref{dualabbreviation}.\tabularnewline
\fielddef{duallongplural} & The plural long form of a dual
abbreviation mapped by \atentryref{dualabbreviation}.\tabularnewline
\fielddef{dualshort} & The short form of a dual
abbreviation mapped by \atentryref{dualabbreviation}.\tabularnewline
\fielddef{dualshortplural} & The plural short form of a dual abbreviation 
mapped by \atentryref{dualabbreviation}.
\end{tabular}
\end{table}
...
the \optref{counter} option).\tabularnewline
\fielddef{group} & The letter group determined by the comparator
(or assigned by the \optref{group} option).\tabularnewline
...
comparator used with the \optref{secondary} sort.\tabularnewline
...
\end{verbatim}
The \optref{secondary} option allows the creation of a fake glossary
...
\subsection{\atentry{abbreviation}}
...
\keyword{primary} entry, and the second is the \keyword{dual} entry
(also referred to as the \keyword{secondary} entry, but is not
related to the \optref{secondary} option). The naming scheme is
\code{@dual}\meta{entry-type} where both the primary and dual are
considered to have the same type of entry (such as
\atentryref{dualsymbol} where both the primary and dual are
functionally like \atentryref{symbol}) or
...
that if you only reference the primary entry (using \cs{gls} etc)
then the dual entry will still be selected if the \optref{selection}
...
\optref{dual-prefix} option and \meta{id} is the label supplied in
the \ext{bib} file.
...
\atentryref{dualindexentry}, \atentryref{dualindexsymbol} 
and \atentryref{index}) and you're not
using the default \optref[combine]{dual-sort}. Remember that the
...
example, don't mix \atentryref{dualindexabbreviation} (duals are
abbreviations) with \atentryref{dualabbreviationentry} (primaries
are abbreviations) when you aren't using \optref[combine]{dual-sort}
(unless you have two different glossaries for the primary vs dual
abbreviations).

Remember that \bibgls\ is designed to take advantage of
\cs{printunsrtglossary}, which simply iterates over all defined
...
\atentryref{dualindexabbreviation} (where the dual is the
abbreviation) and \atentryref{dualabbreviationentry} (where the
...
\end{document}
\end{verbatim}
This uses the default \optref[combine]{sort}, so all the entries are
...
\cs{newabbreviation} automatically assigns the abbreviation 
...
abbreviations created with \atentryref{dualabbreviationentry} and
\verb|\gls{dual.html}| and \verb|\gls{dual.xml}| for the dual
abbreviations created with \atentryref{dualindexabbreviation}. Also
...
\atentryref{tertiaryindexabbreviationentry} instead of
\atentryref{dualabbreviationentry}.

\subsection{\atentry{dualentry}}
\labelatentry{dualentry}

The \atentry{dualentry} entry type is similar to \atentryref{entry} but
...
\end{itemize}
The required fields are as for \atentryref{entry}.
...
\subsection{\atentry{dualindexentry}}
\labelatentry{dualindexentry}

There's only one required field: \field{description}.  The primary
entry behaves like \atentryref{index} and the dual entry behaves
like \atentryref{entry}.  The default field mapping is:
\begin{itemize}
...
\optref{dual-indexentry-backlink} is set). The reason that the primary
(rather than the dual) is like \atentryref{index} is to allow the
primaries to merge with any \atentryref{index} entries found in the
...
(although this may be overridden, for example, by the \optref{category} option).
...
\end{document}
\end{verbatim}
This uses \optref{combine-dual-locations} to combine the locations
for the primary and dual entries so that they only appear in the
index.

\subsection{\atentry{dualindexabbreviation}}
\labelatentry{dualindexabbreviation}

The \atentry{dualindexabbreviation} entry type is similar to 
\atentryref{dualindexentry} and again, by default, the
field mapping is:
\begin{itemize}
\item \field{name} $\mapsto$ \field{name}
\end{itemize}
...
\subsection{\atentry{dualindexsymbol}}
\labelatentry{dualindexsymbol}

The \atentry{dualindexsymbol} entry type is similar to 
\atentryref{dualindexentry}, but by default the
field mappings are:
\begin{itemize}
...
\end{itemize}
The required fields are: \field{symbol} and \field{description}.
...
label prefix and it also switches off the hyperlinks (since it's not
particularly informative to link to the index).
...
for symbols (\cs{glsxtrpostdescsymbol}) to automatically index 
the symbol definition. Similarly for the \code{general}
post-description hook \cs{glsxtrpostdescgeneral}. 
...
\subsection{\atentry{dualindexnumber}}
\labelatentry{dualindexnumber}
...
\subsection{\atentry{dualabbreviationentry}}
\labelatentry{dualabbreviationentry}

The \atentry{dualabbreviationentry} entry type is similar to 
\atentryref{dualentry}, but by default the
field mappings are:
\begin{itemize}
...
\atentryref{dualentryabbreviation}. In version 1.1, it was renamed
...
\subsection{\atentry{dualentryabbreviation}}
\labelatentry{dualentryabbreviation}

This entry type is deprecated as from \bibgls\ version 1.1. It's functionally
equivalent to \atentryref{dualabbreviationentry} but its name
doesn't fit the general dual entry naming scheme.

\subsection{\atentry{dualsymbol}}
\labelatentry{dualsymbol}

This is like \atentryref{dualentry} but the default mappings are:
\begin{itemize}
\item \field{name} $\mapsto$ \field{symbol}
\item \field{plural} $\mapsto$ \field{symbolplural}
\item \field{symbol} $\mapsto$ \field{name}
...
\subsection{\atentry{dualnumber}}
\labelatentry{dualnumber}

This is almost identical to \atentryref{dualsymbol} but entries are
...
\subsection{\atentry{dualabbreviation}}
...
\subsection{\atentry{dualacronym}}
...
\keyword{tertiary} which is effectively an appendage of the
...
primary with \optref[primary]{combine-dual-locations}). The tertiary
...
with \optref{tertiary-type} and the \field{category} with
\optref{tertiary-category}. The label prefix defaults to
\idprefix{tertiary} and can be changed with \optref{tertiary-prefix}.

\subsection{\atentry{tertiaryindexabbreviationentry}}
\labelatentry{tertiaryindexabbreviationentry}

This entry type is very similar to
\atentryref{dualindexabbreviation} but creates a tertiary entry as
well.  The required fields are: \field{short} and \field{long} (as for
\atentryref{dualindexabbreviation}) and also \field{description}.  The
...
\end{verbatim}
The \csref{bibglsnewtertiaryindexabbreviationentrysecondary} command
is provided in the \ext{glstex} file as:
\begin{verbatim}
...
way the entries are sorted and on the \optref{dual-sort} setting.
...
switches on the \styopt{sort=none} package option (if you have a new
...
it's omitted from \cs{newglossaryentry} (or similar commands). The
...
\begin{definition}\plabelcs{GlsXtrLoadResources}
...
later in the document if required. (Similarly if \optref{dual-type}
and \optref{secondary} have been used.)

Rather than directly using \cs{glssetwidest}, \bibgls\ provides
\csref{bibglssetwidest} (where the type isn't known) and 
\cs{bibglssetwidestfortype} (where \cs{glssetwidest} needs to be
appended to the glossary preamble).

Note that if you don't set a separate sort for dual entries with
\optref{dual-sort} then the widest entry name will be determined
...
\subsection{\csoptnv{entry-type-aliases}\csopteqval{\keyvallist}}
\labelopt{entry-type-aliases}
...
abbreviations defined with \atentryref{dualabbreviationentry}
instead of \atentryref{abbreviation}. Instead of editing
...
\optref{category} option except that it sets the \field{type} field.
...
Note that reverse sorts maintain hierarchy, so sub-entries will
still be listed after the parent entry.

Remember that you can have \atentryref{preamble} definitions that
...
\end{verbatim}
This provides the command \csfmt{sortop} for use in the document.
...
\begin{table}[p]
\caption{Summary of Available Sort Options: Rule-Based Collation}
...
locale\tabularnewline
\optfmt{custom} & sort according to \optref[\meta{custom
rule}]{sort-rule}\tabularnewline
\optfmt{custom-reverse} & reverse sort according to \optref[\meta{custom rule}]{sort-rule}
\end{tabular}
\end{table}
...
don't actually perform any sorting.
\begin{itemize}
...
identified with \cs{glssee}) come after entries with records.

\end{itemize}

\subsubsection{Rule-Based Collation}

The sort methods listed in \tableref{tab:sortoptionsrule} use
rule-based comparators, which usually ignore most punctuation and
may ignore modifiers (such as accents). Use with \optref{break-at}
to determine whether or not to split at word boundaries.
...
given by the \gls{IETF} language tag \meta{lang tag}.
...
from the Java Runtime Environment (which usually matches the
...
and lower case are in separate letter groups. For example, the list
\code{antelope}, \code{bee}, \code{Africa}, \code{aardvark} and \code{Brazil} 
will be ordered \code{Africa} (letter group \qt{A}), \code{Brazil}
...
\code{antelope}, \code{bee}, \code{Africa}, \code{aardvark} and \code{Brazil}
will be ordered: \code{aardvark} (letter group \qt{A}), \code{Africa}
...
use a letter-integer hybrid. They behave much like the
...
such as \optfmt{sort=hexadecimal} or \optfmt{sort=float}) but in
addition to recognising all the digits in the Unicode \qt{Number,
Decimal Digit} category it also recognises the subscript and
...
corresponding Devanagari digits. (But note that the non-digit
character comparisons will still be based on their Unicode values
not according to a particular locale. This type of sort method is
intended primarily for symbolic values, such as chemical formulae,
rather than for words or phrases.)

Signed integers are also recognised, so \code{abc-12foo} is less
than \code{abc+6bar}, which is again different from the result
obtained with a straight letter comparator where \code{+} (0x2B)
comes before \code{-} (0x2D). The sign must be followed by at
least one digit for it to be recognised as a number otherwise it's
treated as a punctuation character.

In the situation where only one sub-string is numeric, the
\optref{letter-number-rule} is used to determine the result if the
non-numeric character is considered a letter otherwise
the \optref{letter-number-punc-rule} is used. If both
are letters, then the comparison is as for the
\optfmt{letter-}\emph{modifier} counterpart.
(for example, \optfmt{letternumber-case} will behave like
\optfmt{letter-case}) otherwise if one is a letter and the other
isn't a letter and isn't a number then the 
\optref{letter-number-punc-rule} is used.

The default setting is \optref[before lower]{letter-number-rule} so,
for example, if comparing \code{abcXYfoo} and \code{abc6bar} then
with the case-sensitive or the upper-lower or the lower-upper sorts,
\code{abc6bar} comes after \code{abcXYfoo} (since \code{X} isn't
lower case).  Whereas with the case-insensitive 
\csopt[letternumber-nocase]{sort} \code{abcXYfoo}
would first be converted to \code{abcxyfoo}, which means that
\code{abc6bar} comes before it.
...
\optref{numeric-locale} to set the locale.
...
\optref{numeric-locale} to set the locale and
\optref{numeric-sort-pattern} to set the number pattern.
...
are for dates and times.  Use \optref{date-sort-format}
and \optref{date-sort-locale} to specify the date format and locale.
\begin{itemize}
...
is assumed to be your default locale (as given by the Java Virtual
Machine) but if you are using a different locale this can
be set with \optref{date-sort-locale}. The pattern is assumed
to be the default for that locale but you can change this with
\optref{date-sort-format}. If you provide your
...
both \optref{date-sort-locale} and \optref{date-sort-format}.)
...
If I change the format to \optref[short]{date-sort-format},
...
\subsection{\csoptnv{trim-sort}\csopteqval{boolean}}
\labelopt{trim-sort}

If the interpreter is used to determine the sort value, this setting
determines whether or not the interpreter should trim leading and
trailing spaces. The default setting is \csopt[true]{trim-sort}.

This option automatically sets \optref[\meta{value}]{dual-trim-sort}
and \optref[\meta{value}]{secondary-trim-sort}.
...
\optref[\meta{option}]{dual-break-at} and
\optref[\meta{option}]{secondary-break-at}.
...
This option automatically implements
\optref[\meta{marker}]{dual-break-marker} and
\optref[\meta{marker}]{secondary-break-marker}.
...
This option automatically implements
\optref[\meta{value}]{dual-sort-suffix} and
\optref[\meta{value}]{secondary-sort-suffix}.
...
For the other types of rules (that actually perform some form of
comparison) then you need to guard against duplicates if you have
...
For example, suppose I want to write about \appfmt{makeglossaries}
...
and \csfmt{command} (through \atentryref{preamble}) then it will
...
the suffix marker, see below, is the empty string). With, say,
\optref[.]{sort-suffix-marker} then the sort value would become
...
This uses \app{makeindex}, which puts both entries in the
...
The switch to \app{xindy} just involves adding the \styoptfmt{xindy}
package option:
\begin{verbatim}
\usepackage[xindy,style=indexgroup]{glossaries}
\end{verbatim}
This results in a glossary that only contains one entry,
\csfmt{makeglossaries}, because \app{xindy} merges entries with
duplicate sort values and the sort values end up as duplicates
because \app{xindy} discards the control sequences. Although
...
\end{document}
\end{verbatim}
This uses the default \optref[locale]{sort} which considers \verb|\|
an ignored character, so both \csfmt{makeglossaries} and
...
\code{cs.makeglossaries} is \verb|\makeglossaries|. Note that in
this case \bibgls\ doesn't detect duplicate sort values since it
only uses a simple string comparison to detect duplicates rather
than using the collator.

If I use a letter-based sort rule instead, for example
\optref[letter-nocase]{sort}, then \csfmt{makeglossaries} will be
...
Now let's suppose I prevent \bibgls\ from interpreting the preamble
using \optref[false]{interpret-preamble}:
\begin{verbatim}
...
\optref[unsrt]{sort}, the initial selection order is based on the record
...
\cs{apptoglossarypreamble}). Note that this is
different to using
\begin{verbatim}
\GlsXtrSetDefaultNumberFormat{glsignore}
\end{verbatim}
...
This option automatically implements
\optref[\meta{value}]{dual-sort-suffix-marker} and
\optref[\meta{value}]{secondary-sort-suffix-marker}.
...
This option automatically implements
\optref[\meta{value}]{dual-strength} and
\optref[\meta{value}]{secondary-strength}.
...
This option automatically implements
\optref[\meta{value}]{dual-decomposition} and
\optref[\meta{value}]{secondary-decomposition}.
...
\subsection{\csoptnv{letter-number-rule}\csopteqval{value}}
\labelopt{letter-number-rule}

This option automatically implements
\optref[\meta{value}]{dual-letter-number-rule}
and
\optref[\meta{value}]{secondary-letter-number-rule}.

If you use one of the letter-number sort methods (such as
\optref[letternumber-case]{sort}), then you can determine the
...
characters (including punctuation and spaces).
\end{itemize}
Remember that the case-insensitive letter-number sort methods always
...
Use \optref{letter-number-punc-rule} to determine the relative
position of white space and punctuation.

\subsection{\csoptnv{letter-number-punc-rule}\csopteqval{value}}
\labelopt{letter-number-punc-rule}

This option automatically implements
\optref[\meta{value}]{dual-letter-number-punc-rule}
and
\optref[\meta{value}]{secondary-letter-number-punc-rule}.
...
\code{0} will be distinct from \textsuperscript{1}. However, for 
...
characters belong to the Unicode category \qt{Number, Decimal
Digit}.
...
Note that \optref[first]{letter-number-rule} and
\optref[last]{letter-number-rule} overrides this option when
comparing a number with white space or punctuation.

\subsection{\csoptnv{numeric-sort-pattern}\csopteqval{value}}
\labelopt{numeric-sort-pattern}

If you use the custom \optref[numberformat]{sort} or
\optref[numberformat-reverse]{sort}, you need to 
...
class. You can use \cs{u}\meta{hex} to indicate Unicode characters
by their hexadecimal code. (Remember to use \cs{string} to
protect it from expansion when the options are written to
the \ext{aux} file.)
Use \optref{dual-numeric-sort-pattern} for 
\optref{dual-sort} and \optref{secondary-numeric-sort-pattern} 
for \optref{secondary}.

\subsection{\csoptnv{numeric-locale}\csopteqval{value}}
\labelopt{numeric-locale}

If you use any of the locale-sensitive numeric sort methods,
such as \optref[numeric]{sort}, use this option to set
...
\end{itemize}
Use \optref{dual-numeric-locale} for 
\optref{dual-sort} and \optref{secondary-numeric-locale} 
for \optref{secondary}.

\subsection{\csoptnv{date-sort-locale}\csopteqval{value}}
\labelopt{date-sort-locale}

If you use a date/time sort method, then you can
set the locale used by Java's date-time parser. If omitted,
the locale is obtained from the Java Virtual Machine, which
will usually be your operating system's locale.

The value may be \optfmt{locale}, \optfmt{doc} (use the document's
locale) or a valid language tag identifying the locale (as above).

\subsection{\csoptnv{date-sort-format}\csopteqval{value}}
\labelopt{date-sort-format}

If you use a date/time sort method, then you can set the format
used by Java's date-time parser. If omitted, the default is assumed.
The \meta{value} may be one of:
\begin{itemize}
...
time) information, then it must be used with \optref[date]{sort}
or \optref[date-reverse]{sort}. If the pattern only contains time
(but not date) information, then it must be used with
\optref[time]{sort} or \optref[time-reverse]{sort}. If the pattern
contains both date and time information, then it must be used with
\optref[datetime]{sort} or \optref[datetime-reverse]{sort}.
...
(\optref[false]{interpret-preamble}) and the field
...
value will contain \code{ad} (from \cs{MakeLowercase}) but this can
...
Here's a complete document that changes the group fields to use
the year and era:
\begin{verbatim}
\documentclass{article}
...
The value of \meta{sort} is as for \optref{sort}, but note
that in this case the sort value \optfmt{unsrt} or \optfmt{none}
means to use the same ordering as the primary entries. For
example, with \optref[de-CH-1996]{sort},
\optref[none:copies]{secondary} the \code{copies} list will be
...
In order to switch fields in \cs{printunsrtglossary}, you need at
least v1.21 of \sty{glossaries-extra} which provides
\cs{glsxtrgroupfield} to keep track of the appropriate field label.
...
\subsection{\csoptnv{secondary-trim-sort}\csopteqval{boolean}}
\labelopt{secondary-trim-sort}

As \optref{trim-sort} but for secondary sorting.
...
\subsection{\csoptnv{secondary-break-at}\csopteqval{value}}
\labelopt{secondary-break-at}

As \optref{break-at} but for secondary entries.

\subsection{\csoptnv{secondary-break-marker}\csopteqval{value}}
\labelopt{secondary-break-marker}

As \optref{break-marker} but for secondary entries.

\subsection{\csoptnv{secondary-sort-suffix}\csopteqval{value}}
\labelopt{secondary-sort-suffix}

As \optref{sort-suffix} but for secondary entries.

\subsection{\csoptnv{secondary-sort-suffix-marker}\csopteqval{value}}
\labelopt{secondary-sort-suffix-marker}

As \optref{sort-suffix-marker} but for secondary entries.

\subsection{\csoptnv{secondary-strength}\csopteqval{value}}
\labelopt{secondary-strength}

As \optref{strength} but for secondary entries.

\subsection{\csoptnv{secondary-decomposition}\csopteqval{value}}
\labelopt{secondary-decomposition}

As \optref{decomposition} but for secondary entries.

\subsection{\csoptnv{secondary-letter-number-rule}\csopteqval{value}}
\labelopt{secondary-letter-number-rule}

As \optref{letter-number-rule} but for secondary letter-number sorting.

\subsection{\csoptnv{secondary-letter-number-punc-rule}\csopteqval{value}}
\labelopt{secondary-letter-number-punc-rule}

As \optref{letter-number-punc-rule} but for secondary letter-number sorting.

\subsection{\csoptnv{secondary-numeric-sort-pattern}\csopteqval{value}}
\labelopt{secondary-numeric-sort-pattern}

As \optref{numeric-sort-pattern} but for secondary
locale-sensitive numeric sorting.

\subsection{\csoptnv{secondary-numeric-locale}\csopteqval{value}}
\labelopt{secondary-numeric-locale}

As \optref{numeric-locale} but for secondary
locale-sensitive numeric sorting.

\subsection{\csoptnv{secondary-date-sort-locale}\csopteqval{value}}
\labelopt{secondary-date-sort-locale}

As \optref{date-sort-locale} but for secondary date-time sorting.

\subsection{\csoptnv{secondary-date-sort-format}\csopteqval{value}}
\labelopt{secondary-date-sort-format}

As \optref{date-sort-format} but for secondary date-time sorting.
...
\subsubsection{\csoptnv{dual-prefix}\csopteqval{value}}
...
\subsubsection{\csoptnv{combine-dual-locations}\csopteqval{value}}
\labelopt{combine-dual-locations}
...
entries as the hyperlink created by \cs{gls} will point to the
...
\subsubsection{\csoptnv{dual-type}\csopteqval{value}}
\labelopt{dual-type}
...
\atentryref{dualentry}, the \field{type} will be set to
\optfmt{dualentry}.
...
set with \optref{type}). If the primary entry doesn't have the
...
\subsubsection{\csoptnv{dual-category}\csopteqval{value}}
...
type. For example, if the entry was defined with
\atentryref{dualentry}, the \field{category} will be set to
\optfmt{dualentry}.
...
may have been set with \optref{category}). If the primary entry
...
\subsubsection{\csoptnv{dual-short-case-change}\csopteqval{value}}
\labelopt{dual-short-case-change}

As \optref{short-case-change} but applies to the \field{dualshort}
field instead.

\subsubsection{\csoptnv{dual-field}\csopteqval{value}}
\labelopt{dual-field}
...
\subsubsection{\csoptnv{dual-sort}\csopteqval{value}}
\labelopt{dual-sort}
...
\subsubsection{\csoptnv{dual-sort-field}\csopteqval{value}}
...
\subsection{\csoptnv{dual-trim-sort}\csopteqval{boolean}}
\labelopt{dual-trim-sort}

As \optref{trim-sort} but for dual sorting.

\subsubsection{\csoptnv{dual-sort-rule}\csopteqval{value}}
\labelopt{dual-sort-rule}

As \optref{sort-rule} but for \csopt[custom]{dual-sort}.

\subsubsection{\csoptnv{dual-break-at}\csopteqval{value}}
\labelopt{dual-break-at}

As \optref{break-at} but for dual entries.

\subsubsection{\csoptnv{dual-break-marker}\csopteqval{value}}
\labelopt{dual-break-marker}

As \optref{break-marker} but for dual entries.

\subsubsection{\csoptnv{dual-sort-suffix}\csopteqval{value}}
\labelopt{dual-sort-suffix}

As \optref{sort-suffix} but for dual entries.

\subsubsection{\csoptnv{dual-sort-suffix-marker}\csopteqval{value}}
\labelopt{dual-sort-suffix-marker}

As \optref{sort-suffix-marker} but for dual entries.

\subsubsection{\csoptnv{dual-strength}\csopteqval{value}}
\labelopt{dual-strength}

As \optref{strength} but for dual entries.

\subsubsection{\csoptnv{dual-decomposition}\csopteqval{value}}
\labelopt{dual-decomposition}

As \optref{decomposition} but for dual entries.

\subsubsection{\csoptnv{dual-letter-number-rule}\csopteqval{value}}
\labelopt{dual-letter-number-rule}

As \optref{letter-number-rule} but for dual entries that use a
letter-number sort.

\subsubsection{\csoptnv{dual-letter-number-punc-rule}\csopteqval{value}}
\labelopt{dual-letter-number-punc-rule}

As \optref{letter-number-punc-rule} but for dual entries that use a
letter-number sort.

\subsubsection{\csoptnv{dual-numeric-sort-pattern}\csopteqval{value}}
\labelopt{dual-numeric-sort-pattern}

As \optref{numeric-sort-pattern} but for dual entries that use a
locale-sensitive numeric sort.

\subsubsection{\csoptnv{dual-numeric-locale}\csopteqval{value}}
\labelopt{dual-numeric-locale}

As \optref{numeric-locale} but for dual entries that use a
locale-sensitive numeric sort.

\subsubsection{\csoptnv{dual-date-sort-locale}\csopteqval{value}}
\labelopt{dual-date-sort-locale}

As \optref{date-sort-locale} but for dual entries that
use a date/time sort.

\subsubsection{\csoptnv{dual-date-sort-format}\csopteqval{value}}
\labelopt{dual-date-sort-format}

As \optref{date-sort-format} but for dual entries that
use a date/time sort.

\subsection{Dual Mappings}
\label{sec:dualoptsmap}

\subsubsection{\csoptnv{dual-entry-map}=\marg{\margm{list1},\margm{list2}}}
\labelopt{dual-entry-map}

This setting governs the behaviour of \atentryref{dualentry}
...
\subsubsection{\csoptnv{dual-abbrv-map}=\marg{\margm{list1},\margm{list2}}}
\labelopt{dual-abbrv-map}

This is like \csopt{dual-entry-map} but applies to
\atentryref{dualabbreviation} rather than \atentry{dualentry}. 
...
\subsubsection{\csoptnv{dual-abbrventry-map}=\marg{\margm{list1},\margm{list2}}}
\labelopt{dual-abbrventry-map}

This is like \csopt{dual-entry-map} but applies to
\atentryref{dualabbreviationentry} rather than \atentry{dualentry}. 
Note that the \field{alias} field can't be mapped. The
default setting is:
\begin{verbatim}
dual-abbrventry-map=
...
\end{verbatim}
See \atentryref{dualabbreviationentry} for further details.


\subsubsection{\csoptnv{dual-symbol-map}=\marg{\margm{list1},\margm{list2}}}
\labelopt{dual-symbol-map}

This is like \csopt{dual-entry-map} but applies to
\atentryref{dualsymbol} rather than \atentry{dualentry}. 
Note that the \field{alias} field can't be mapped. The
default setting is:
\begin{verbatim}
dual-symbol-map=
...
\subsubsection{\csoptnv{dual-indexentry-map}=\marg{\margm{list1},\margm{list2}}}
\labelopt{dual-indexentry-map}

This is like \csopt{dual-entry-map} but applies to
\atentryref{dualindexentry} rather than \atentry{dualentry}. 

\subsubsection{\csoptnv{dual-indexsymbol-map}=\marg{\margm{list1},\margm{list2}}}
\labelopt{dual-indexsymbol-map}

This is like \csopt{dual-entry-map} but applies to both
\atentryref{dualindexsymbol} and \atentryref{dualindexnumber}. 

\subsubsection{\csoptnv{dual-indexabbrv-map}=\marg{\margm{list1},\margm{list2}}}
\labelopt{dual-indexabbrv-map}

This is like \csopt{dual-entry-map} but applies to both
\atentryref{dualindexabbreviation} and
\atentryref{tertiaryindexabbreviationentry}. 

\subsection{Dual Back-Links}
\label{sec:dualoptsbacklinks}

\subsubsection{\csoptnv{dual-entry-backlink}\csopteqval{boolean}}
\labelopt{dual-entry-backlink}

This is a boolean setting.
When used with \atentryref{dualentry}, if \meta{boolean} is
...
\subsubsection{\csoptnv{dual-abbrv-backlink}\csopteqval{boolean}}
\labelopt{dual-abbrv-backlink}

This is analogous to \csopt{dual-entry-backlink} but for entries
defined with \atentryref{dualabbreviation} instead of
\atentry{dualentry}.

\subsubsection{\csoptnv{dual-symbol-backlink}\csopteqval{boolean}}
\labelopt{dual-symbol-backlink}

This is analogous to \csopt{dual-entry-backlink} but for entries
defined with \atentryref{dualsymbol} instead of \atentry{dualentry}.

\subsubsection{\csoptnv{dual-abbrventry-backlink}\csopteqval{boolean}}
\labelopt{dual-abbrventry-backlink}

This is analogous to \csopt{dual-entry-backlink} but for entries
defined with \atentryref{dualabbreviationentry} instead of
\atentry{dualentry}.

\subsubsection{\csoptnv{dual-indexentry-backlink}\csopteqval{boolean}}
\labelopt{dual-indexentry-backlink}

This is analogous to \csopt{dual-entry-backlink} but for entries
defined with \atentryref{dualindexentry} instead of
\atentry{dualentry}.

\subsubsection{\csoptnv{dual-indexsymbol-backlink}\csopteqval{boolean}}
\labelopt{dual-indexsymbol-backlink}

This is analogous to \csopt{dual-entry-backlink} but for entries
defined with \atentryref{dualindexsymbol} and
\atentryref{dualindexnumber}.

\subsubsection{\csoptnv{dual-indexabbrv-backlink}\csopteqval{boolean}}
\labelopt{dual-indexabbrv-backlink}

This is analogous to \csopt{dual-entry-backlink} but for entries
defined with \atentryref{dualindexabbreviation} and
\atentryref{tertiaryindexabbreviationentry}.

\subsubsection{\csoptnv{dual-backlink}\csopteqval{boolean}}
...
\csopt[\meta{boolean}]{dual-indexsymbol-backlink}, and
\csopt[\meta{boolean}]{dual-indexabbrv-backlink}.

\section{Tertiary Entries}

\subsection{\csoptnv{tertiary-prefix}\csopteqval{value}}
\labelopt{tertiary-prefix}
...
\subsection{\csoptnv{tertiary-type}\csopteqval{value}}
\labelopt{tertiary-type}
...
\subsection{\csoptnv{tertiary-category}\csopteqval{value}}
\labelopt{tertiary-category}
...
\subsection{\csfmt{bibglsnewdualindexentry}}
\labelcs{bibglsnewdualindexentry}

\begin{definition}
\cs{bibglsnewdualindexentry}\margm{label}\margm{options}\margm{name}\margm{description}
\end{definition}
This command is used to define primary terms identified with the
\atentryref{dualindexentry} type. The definition provided in the \ext{glstex}
file is:
\begin{verbatim}
...
\subsection{\csfmt{bibglsnewdualindexentrysecondary}}
\labelcs{bibglsnewdualindexentrysecondary}

\begin{definition}
\cs{bibglsnewdualindexentrysecondary}\margm{label}\margm{options}\margm{name}\margm{description}
\end{definition}
...
\subsection{\csfmt{bibglsnewdualindexsymbol}}
\labelcs{bibglsnewdualindexsymbol}

\begin{definition}
\cs{bibglsnewdualindexsymbol}\margm{label}\margm{options}\margm{name}\marg{symbol}\margm{description}
\end{definition}
This command is used to define primary terms identified with the
\atentryref{dualindexsymbol} type. The definition provided in the \ext{glstex}
file is:
\begin{verbatim}
...
\subsection{\csfmt{bibglsnewdualindexsymbolsecondary}}
\labelcs{bibglsnewdualindexsymbolsecondary}

\begin{definition}
\cs{bibglsnewdualindexsymbolsecondary}\margm{label}\margm{options}\margm{name}\margm{description}
\end{definition}
This command is used to define secondary terms identified with the
\atentryref{dualindexsymbol} type. The definition provided in the \ext{glstex}
file is:
...
\providecommand{\bibglsnewdualindexsymbolsecondary}[5]{%
  \longnewglossaryentry*{#1}{name={#3},category={symbol},symbol={#4},#2}{#5}%
}
\end{verbatim}

\subsection{\csfmt{bibglsnewdualindexnumber}}
\labelcs{bibglsnewdualindexnumber}

\begin{definition}
\cs{bibglsnewdualindexnumber}\margm{label}\margm{options}\margm{name}\marg{symbol}\margm{description}
\end{definition}
This command is used to define primary terms identified with the
\atentryref{dualindexnumber} type. The definition provided in the \ext{glstex}
file is:
\begin{verbatim}
...
\subsection{\csfmt{bibglsnewdualindexnumbersecondary}}
\labelcs{bibglsnewdualindexnumbersecondary}

\begin{definition}
\cs{bibglsnewdualindexnumbersecondary}\margm{label}\margm{options}\margm{name}\margm{description}
\end{definition}
This command is used to define secondary terms identified with the
\atentryref{dualindexnumber} type. The definition provided in the \ext{glstex}
file is:
...
\providecommand{\bibglsnewdualindexnumbersecondary}[5]{%
  \longnewglossaryentry*{#1}{name={#3},category={number},symbol={#4},#2}{#5}%
}
\end{verbatim}

\subsection{\csfmt{bibglsnewdualindexabbreviation}}
\labelcs{bibglsnewdualindexabbreviation}

\begin{definition}
\cs{bibglsnewdualindexabbreviation}\margm{label}\margm{dual-label}\margm{options}\margm{name}\margm{short}\margm{long}\margm{description}
\end{definition}
This command is used to define primary terms identified with the
\atentryref{dualindexabbreviation} type. The definition provided in the \ext{glstex}
file is:
\begin{verbatim}
...
Note that the \field{name} uses \plabelcs{bibglsuseabbrvfont}
\begin{definition}
\cs{bibglsuseabbrvfont}\margm{text}\margm{category}
\end{definition}
to format the name, which ensures that it uses the same font as the
short form for the dual abbreviation. This will use
\cs{glsuseabbrvfont} (provided by \sty{glossaries-extra} version
1.21) if it's defined otherwise it will be defined to replicate that
command.

\subsection{\csfmt{bibglsnewdualindexabbreviationsecondary}}
\labelcs{bibglsnewdualindexabbreviationsecondary}

\begin{definition}
\cs{bibglsnewdualindexabbreviationsecondary}\margm{label}\margm{options}\margm{name}\margm{short}\margm{long}\margm{description}
\end{definition}
This command is used to define secondary terms identified with the
\atentryref{dualindexabbreviation} entry type.
...
\subsection{\csfmt{bibglsnewdualabbreviationentry}}
\labelcs{bibglsnewdualabbreviationentry}

\begin{definition}
\cs{bibglsnewdualabbreviationentry}\margm{label}\margm{options}\margm{short}\margm{long}\margm{description}
\end{definition}
This command is used to define primary terms identified with the
\atentryref{dualabbreviationentry} type. The definition provided in the \ext{glstex}
file is:
\begin{verbatim}
...
\subsection{\csfmt{bibglsnewdualabbreviationentrysecondary}}
\labelcs{bibglsnewdualabbreviationentrysecondary}

\begin{definition}
\cs{bibglsnewdualabbreviationentrysecondary}\margm{label}\margm{options}\margm{short}\margm{long}\margm{description}
\end{definition}
This command is used to define secondary terms identified with the
\atentryref{dualabbreviationentry} type. The definition provided in the \ext{glstex}
file is:
...
\subsection{\csfmt{bibglsnewtertiaryindexabbreviationentry}}
\labelcs{bibglsnewtertiaryindexabbreviationentry}

\begin{definition}
\cs{bibglsnewtertiaryindexabbreviationentry}\margm{label}\margm{dual-label}\margm{options}\margm{name}\margm{short}\margm{long}\margm{description}
\end{definition}
This command is used to define primary terms identified with the
\atentryref{tertiaryindexabbreviationentry} type. It's essentially
the same as \csref{bibglsnewdualindexabbrevation}.
...
\subsection{\csfmt{bibglsnewtertiaryindexabbreviationentrysecondary}}
\labelcs{bibglsnewtertiaryindexabbreviationentrysecondary}

\begin{definition}
\cs{bibglsnewtertiaryindexabbreviationentrysecondary}\margm{label}\margm{tertiary-label}\margm{options}\margm{tertiary
opts}\margm{primary name}\margm{short}\margm{long}\margm{description}
\end{definition}
This command is used to define secondary and tertiary terms identified with the
\atentryref{tertiaryindexabbreviationentry} type. The secondary
...
\plabelcs{bibglsuselongfont}
\begin{definition}
\cs{bibglsuselongfont}\margm{text}\margm{category}
\end{definition}
to format the name, which ensures that it uses the same font as the
long form for the dual abbreviation. This will use
\cs{glsuselongfont} (provided by \sty{glossaries-extra} version
1.21) if it's defined otherwise it will be defined to replicate that
command.
...
\subsection{\csfmt{bibglsdatetimegroup}}
\labelcs{bibglsdatetimegroup}

\begin{definition}
\cs{bibglsdatetimegroup}\margm{YYYY}\margm{MM}\margm{DD}\margm{hh}\margm{mm}\margm{ss}\margm{zone}\margm{title}\margm{group id}\margm{type}
\end{definition}

This command is used for group labels with date-time sorting (for example,
\optref[datetime]{sort}). This has ten arguments, which means a
...
\subsection{\csfmt{bibglsdatetimegrouptitle}}
\labelcs{bibglsdatetimegrouptitle}

\begin{definition}
\cs{bibglsdatetimegrouptitle}\margm{YYYY}\margm{MM}\margm{DD}\margm{hh}\margm{mm}\margm{ss}\margm{zone}\margm{title}\margm{group id}\margm{type}
\end{definition}

This command is used for group titles with date-time sorting.
The default definition is
\begin{verbatim}
...
\subsection{\csfmt{bibglsdategroup}}
\labelcs{bibglsdategroup}

\begin{definition}
\cs{bibglsdategroup}\margm{YYYY}\margm{MM}\margm{DD}\margm{G}\margm{title}\margm{group id}\margm{type}
\end{definition}

This command is used for group labels with date sorting (for
example, with \optref[date]{sort}). The default definition is
\begin{verbatim}
...
\subsection{\csfmt{bibglsdategrouptitle}}
\labelcs{bibglsdategrouptitle}

\begin{definition}
\cs{bibglsdategrouptitle}\margm{YYYY}\margm{MM}\margm{DD}\margm{G}\margm{title}\margm{group id}\margm{type}
\end{definition}
This command is used for group titles with date sorting.
The default definition is
\begin{verbatim}
...
\subsection{\csfmt{bibglstimegroup}}
\labelcs{bibglstimegroup}

\begin{definition}
\cs{bibglstimegroup}\margm{hh}\margm{mm}\margm{ss}\margm{zone}\margm{title}\margm{group id}\margm{type}
\end{definition}

This command is used for group labels with time sorting (for example,
\optref[time]{sort}).

\subsection{\csfmt{bibglstimegrouptitle}}
\labelcs{bibglstimegrouptitle}

\begin{definition}
\cs{bibglstimegrouptitle}\margm{hh}\margm{mm}\margm{ss}\margm{zone}\margm{title}\margm{group id}\margm{type}
\end{definition}

This command is used for group titles with time sorting.
...
\subsection{\csfmt{bibglssetwidest}}
\labelcs{bibglssetwidest}

\begin{definition}
\cs{bibglssetwidest}\margm{level}\margm{name}
\end{definition}

This is used by \optref{set-widest} to set the widest name for the
...
\subsection{\csfmt{bibglssetwidestfortype}}
\labelcs{bibglssetwidestfortype}

\begin{definition}
\cs{bibglssetwidestfortype}\margm{type}\margm{level}\margm{name}
\end{definition}
...
sort value than either \app{makeindex} or \app{xindy}.
...
\begin{alltt}
\atentryref{dualabbreviationentry}\marg{\meta{label},
  short = \margm{short},
  long = \margm{long},
  description = \margm{description},
  \meta{\keyvallist}
