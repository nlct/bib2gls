...
The \atentry{author} entry type is primarily provided for use 
...
\csref{bibglsnewauthor}.

\entrysection{editor}

The \atentry{editor} entry type is primarily provided for use 
by the \atentry{bibtexentry} type. You may use it explicitly
if you want, but you need to take care that it doesn't clash with
\atentry{bibtexentry}. It behaves much like \atentry{index} except that the
term is written to the output file using the command
\csref{bibglsneweditor}.
...
entries. A multi-entry type may not spawn additional entries
depending on the supplied field values.
...
basic \BibTeX's entry types. If you use 
...
You will also need to provide field aliases. For example:
\begin{codeenv}
\csopt[\marg{
  title=name
}]{field-aliases}
...
(or provide the \field{bibtextype} key with \cs{glsaddkey} etc).

The \atentry{bibtexentry} essentially creates an \atentry{index}
form of entry, but it additionally defines an \atentry{author}
or \atentry{editor} entry for each listed author or editor
and makes the \atentry{bibtexentry} dependent on the 
author and editor entries and vice versa. Each author is effectively
defined as
\begin{codeenv}
\atentry{author}\marg{\meta{label},
  name=\marg{\ics{bibglscontributor}\margm{forenames}\margm{von}\margm{surname}\margm{suffix}}
}
\end{codeenv}
and each editor is effectively defined as
\begin{codeenv}
\atentry{editor}\marg{\meta{label},
  name=\marg{\ics{bibglscontributor}\margm{forenames}\margm{von}\margm{surname}\margm{suffix}}
}
\end{codeenv}
In both cases, the label is obtained by converting the \field{name}
...
\begin{codeenv}
\ics{glsxtrfieldlistadd}\margm{id}\marg{bibtex\meta{contributor-type}}\margm{contributor-id}
\end{codeenv}
where \meta{id} is the label identifying the main
\atentry{bibtexentry}, \meta{contributor-type} is either
\code{author} or \code{editor} and \meta{contributor-id} is the
label identifier the contributor.

Each author is defined using
\ics{bibglsnewauthor} and each editor is defined using
\ics{bibglsneweditor}. In both cases the definition is followed
by:
\begin{codeenv}
...
\item \optfmt{ignore}: this is equivalent to the default
...
will become \code{DickPhilipK} but the original value is retained
when \csopt[create]{missing-parents} is set. Once all the entries
have been processed, if \code{ubik} has been selected but no entry
can be found with the label \code{DickPhilipK} then a new entry will
be created as though it had been defined with:
\begin{verbatim}
@index{DickPhilipK,
...
(lower case and without the initial \code{@});

\item \optfmt{same as original entry}: set the \field{category} 
...
\cssection{bibglsnewauthor}

\formatdef{bibglsnewauthor}
This command is used to define terms identified with
\atentry{author} (typically implicitly created through
\atentry{bibtexentry}).

\cssection{bibglsneweditor}

\formatdef{bibglsneweditor}
This command is used to define terms identified with
\atentry{editor} (typically implicitly created through
\atentry{bibtexentry}).
