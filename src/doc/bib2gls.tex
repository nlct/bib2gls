...
\subsection{\csopt{type}=\margm{label}}
\labelopt{type}

The \csopt{type} option indicates that all the entries must have the
\field{type} field set to \meta{label}. This will override any
\field{type} fields supplied in the \ext{bib} file
...
\subsection{\csopt{category}=\margm{label}}
\labelopt{category}

The \csopt{category} option indicates that all the entries must have the
\field{category} field set to \meta{label}. This will override any
...
should begin with \csref{bibglsprefix}\meta{n}. The \meta{value} may
...
\cs{providecommand}\marg{\cs{bibglsprefix}}[1]\marg{\%
  \cs{ifcase}\string#1
  \cs{or} \meta{prefix-1}\csref{bibglspostprefix}
  \cs{or} \meta{prefix-2}\cs{bibglspostprefix}
  \ldots
  \cs{else} \meta{prefix-n}\cs{bibglspostprefix}
  \cs{fi}
}
\end{alltt}
...
This option sets the \field{type} field to \meta{value} for all dual
...
file (or created through a mapping).

\subsection{\csopt{dual-category}=\margm{value}}
\labelopt{dual-category}

This option sets the \field{category} field to \meta{value} for all dual
...
provided. If the glossary type has been provided by \csoptref{type}
(and \csoptref{dual-type} if there are any dual entries) then the
definition of \cs{bibglsprefix} will be appended to the glossary
