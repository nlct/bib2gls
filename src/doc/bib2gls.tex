...
German: \gls{de.rna}; \gls{de.rna}.
\end{verbatim}
Another variation is to use the \optfmt{long-short-user}
...
Here's an example to be used with the \optfmt{long-short-desc}
style:
\begin{alltt}
@dualabbreviation\marg{rna,
  short=\marg{RNA},
...
\end{document}
\end{verbatim}
Note that since this document uses the \optfmt{long-short-desc}
...
An alternative is to use the \optfmt{long-short-user-desc} style
...
\optfmt{long-postshort-user-desc} style which defers the
...
\end{verbatim}
The \qt{sc} styles (such as \optfmt{long-short-sc}) use a different
definition:
\begin{verbatim}
...
formation. The first is \optfmt{aposplural} which uses the suffix
\begin{verbatim}
'\abbrvpluralsuffix
\end{verbatim}
That is, an apostrophe followed by \cs{abbrvpluralsuffix} is
appended. The second attribute is \optfmt{noshortplural} which
...
\end{definition}
This command is used to define terms identified with the
\atentry{dualabbreviation} type where the \field{description} field
is swapped with the \field{long} field. The definition provided in the
\ext{glstex} file is:
\begin{verbatim}
\providecommand{\bibglsnewdualabbreviation}[4]{%
  \newabbreviation[#2,description={#4}]{#1}{#3}{#4}%
}
\end{verbatim}
Note that this ensures that the \field{description} is set to the
\field{long} value. If the \field{description} should not be the
same as the \field{long} field, make sure you change the mapping
using \optref{dual-abbrv-map} so that the \field{description} and
\field{long} fields aren't swapped.

\section{\cs{bibglsnewdualabbreviationdesc}}
\labelcs{bibglsnewdualabbreviationdesc}

\begin{definition}
\cs{bibglsnewdualabbreviationdesc}\margm{label}\margm{options}\margm{short}\margm{long}
\end{definition}
This command is used to define terms identified with the
\atentry{dualabbreviation} type where the \field{description} field
is not swapped with the \field{long} field. The definition provided in the
\ext{glstex} file is:
\begin{verbatim}
\providecommand{\bibglsnewdualabbreviationdesc}[4]{%
  \newabbreviation[#2]{#1}{#3}{#4}%
}
\end{verbatim}
