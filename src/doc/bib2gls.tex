...
\ext{aux} file as though it was actually an ignored record:
\begin{codeenv}
...
updated, and
the \field{bibtexentry} and \field{bibtexentry@entry-type} internal
...
has a record then all the linked \atentry{bibtexentries} will
be selected if \csopt[recorded and deps and see]{selection}).
You can instruct \bibgls\ to treat \ics{citation} as an ignored 
record using \longarg{cite-as-record}.
