...
\newcommand*{\printcommandsummary}{%
  ...
  {%
   \def\glossarypreamble{\texparserdefnotetarget}%
...
\printfields
 [%
  ...
  set in the resource command or by \bibgls.%
 ]%
 {internalfield}%
...
must be set and must contain a least one label. This field is not
available for use in any other context. 
...
\end{codeenv}
This creates the main \meta{id} entry, which contains all the fields
(except for \field{adoptparents}) that were in the original
\atentry{progenitor} definition and has the new field \field{progeny}
set. The main entries are defined in the \ext{glstex} file with
\gls{bibglsnewprogenitor}.

In addition to the main \meta{id} entry, this also creates the
spawned entries \code{\meta{parent-1 id}.\meta{id}}, \ldots, 
\code{\meta{parent-N id}.\meta{id}} that are dependent on the main
\meta{id} entry.  The only fields that aren't copied from the original
\atentry{progenitor} to the spawned entries are the
\field{adoptparents} field (required in the original),
and the \field{alias} and \field{parent} fields (if set).

The spawned entries have the \field{parent} field set to the
corresponding label obtained from the \field{adoptparents} list. This parent
entry must also be defined, as usual for the \field{parent} field.
The spawned entry type needs to match the \field{parent}. The main
entry can be assigned to a different type with the
\csopt{progenitor-type} option. The spawned entries are defined in
the \ext{glstex} file with \gls{bibglsnewspawnedindex}

If the main entry is referenced in the document then (assuming the
default selection criteria) the spawned entries will also be automatically
selected. You can check for the existence of the \field{progenitor}
field using \cs{glsxtrifhasfield} and fetch the \field{location}
field from the main entry, if required.
...
The following \atentryfmt{spawn\meta{type}} commands are all forms
of \atentry{progenitor} that create the given
\atentryfmt{\meta{type}} of entry. The spawned entries are actually
created with the entry type \atentryfmt{spawned\meta{type}}. In the
...
\atentryfmt{spawned\meta{type}} as the entry type.
...
progeny, which need to be in the same glossary as their
