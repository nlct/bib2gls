...
\newcommand*{\csref}[1][noindex=false]{\gls[#1]}
...
\styfmt{glossaries-extra}'s \csref{GlsXtrLoadResources} command. When used
...
\strut
\cmd{usepackage}\oarg{\styoptref{record}}\marg{glossaries-extra}
\strut
...
option instead.
\begin{codeenv}
\cmd{usepackage}\oarg{\styopt{record},\styopt{accsupp}}\marg{glossaries-extra}
\end{codeenv}
\end{table}
...
With \code{\csref{glsxtrresourcefile}\oargm{options}\margm{filename}}, if 
...
actually define the entries (such as \csref{bibglsnewindex}).
...
field. There are two forms of modification: the field contents are
modified so that the value is encapsulated by a command or \bibgls\
will perform the actual \idx{case-change} according to its own
algorithm. The results can vary according to the field content.

Each of the case-changing resource options may take one of the
following values:
\begin{itemize}
\item \optfmt{none}: don't apply any case-changing (default);
\item \optfmt{lc-cs}: convert to \idx{lowercase} by encapsulating
the field value with \ics{MakeTextLowercase};
\item \optfmt{uc-cs}: convert to \idx{uppercase} by encapsulating
the field value with \ics{MakeTextUppercase};
\item \optfmt{firstuc-cs}: convert to first letter \idx{uppercase}
by encapsulating the field value with \ics{makefirstuc};
\item \optfmt{title-cs}: convert to \idx{titlecase} 
by encapsulating the field value with \ics{capitalisewords};
\item \optfmt{lc}: convert to \idx{lowercase} by making the
appropriate modifications to tokens in the field value that have
a known \idx{lowercase} alternative;
\item \optfmt{uc}: convert to \idx{uppercase} by making the
appropriate modifications to tokens in the field value that have
a known \idx{uppercase} alternative;
...
The \optfmt{\meta{option}-cs} values defer the actual case-changing
to \TeX. Be aware of the limitations of using any of the
...
\sty{textcase}) to prevent any case-change to \meta{content}.

For the options where \bibgls\ itself performs the case-change, then
...
be replaced with the corresponding upper/lower case character.
For \optfmt{title} and \optfmt{firstuc}, the title case character is
...
is used as the replacement. Most characters have the same upper and
title case alternative (for example, \qt{a} will be converted to \qt{A}
for the \optfmt{title}, \optfmt{firstuc} and \optfmt{uc} options) but
some characters have different title/upper versions (for example, 
...
then all remaining tokens are skipped, so no case-change will be
performed.

\item\label{case-change-group} If a group \code{\margm{content}} is
found, then the case-change is applied to the entire \meta{content}
...
\item If a control sequence \csfmt{}\meta{csname} is found, then:
  ...
  it's replaced with its case-change counterpart (if not already the
  correct case).
  ...
  for a case-change). This is different to the way \ics{makefirstuc}
  ...
  If the option is \optfmt{firstuc} then all the remaining tokens are
  skipped (so no case-change will be performed). If the option is 
  \optfmt{title} then the subsequent tokens
  are skipped until a word-boundary is found (so no case-change is
  ...
  \item If the control sequence is \ics{glsentrytitlecase} then:
   ...
   \end{description}
  (The field and entry labels are skipped.)

  \item If the control sequence is \ics{glshyperlink} then the
  case-change is applied to its optional argument. (If there was no
  ...
  \ics{dglsdisp} or \ics{dglslink} then the case-change will be
  applied to the appropriate argument.
  ...
  \item If the control sequence is followed by a group, then the
   appropriate case-change is applied to the group contents.
   Unlike step~\ref{case-change-group}, the case-change isn't
...
For the \optfmt{title} option, a word-boundary is identified if a
token is encountered that is a white space character or is
\ics{space} or \ics{cs.space}.
...
\begin{codeenv}
\atentry{abbreviation}\marg{html,
 \field{short} = \marg{\cs{MakeTextLowercase}\marg{HTML}},
 \field{long}  = \marg{\cs{capitalisewords}\marg{hypertext markup language}},
 \field{description}=\marg{\cs{makefirstuc}\marg{a markup language for creating web pages}}
}
\end{codeenv}
...
\begin{codeenv}
\cs{MakeTextUppercase}\marg{z\cs{ae}\cs{oe}}
\end{codeenv}
...
\end{codeenv}
then the result is just \code{html} (since the case change is
...
is stripped from the format (encap) information, so you just get the
start and end locations added as individual elements. Each item in the 
list is provided
in the form:
\begin{codeenv}
...
initial marker is stripped from the location formatting commands,
such as \gls{glsnoidxdisplayloc}, to allow for easy conversion to the
...
converted to the default \code{glsnumberformat} (the records in the
...
\idx{locationlist}.
\end{itemize}
The primary locations are identified with
\csopt{primary-location-formats}, which must be set if the
default \csopt[false]{save-primary-locations} is changed.

The primary locations are copied to the \field{primarylocations}
field and encapsulated with \csref{bibglsprimary}.
...
style. For example, the \glostyle{bookindex} style inserts
...
\begin{codeenv}
\csfmt{documentclass}\marg{report}

\cs{usepackage}\oarg{colorlinks}\marg{hyperref}
\cs{usepackage}\oarg{record,
 postpunc=\marg{dot},
 nostyles,
 stylemods=\marg{tree,bookindex},
 style=bookindex}\marg{glossaries-extra}
...
\gls{glsxtrnewglslike}\oarg{format=hyperbf}\marg{}\marg{\csfmt{primary}}\marg{\csfmt{primarypl}}\marg{\csfmt{Primary}}\marg{\csfmt{Primarypl}}

\csfmt{begin}\marg{document}
\cs{chapter}\marg{Sample}
\csfmt{Primary}\marg{waterfowl}, \cs{gls}\marg{bird} and \cs{gls}\marg{zebra}.

\cs{chapter}\marg{Another Sample}
\cs{Gls}\marg{waterfowl}, \csfmt{primary}\marg{bird} and \cs{gls}\marg{zebra}.

\cs{chapter}\marg{Yet Another Sample}
\cs{Gls}\marg{waterfowl}, \cs{gls}\marg{bird} and \csfmt{primary}\marg{zebra}.

\cs{chapter}\marg{Yet Another Sample Again}
\cs{Gls}\marg{waterfowl}, \cs{gls}\marg{bird}, \csfmt{primarypl}\marg{parrot} and \cs{gls}\marg{zebra}.

\ics{printunsrtglossary*}\oarg{style=tree,nonumberlist}\marg{\comment{}
 \cs{renewcommand}*\marg{\ics{glsextrapostnamehook}}[1]\marg{\ics{glsadd}\oarg{format=hyperemph}\marg{\idx{param}1}}\comment{}
}
...
The \csopt[hyperbf,hyperemph]{primary-location-formats} setting in
the above indicates that locations encapsulated with \ics{hyperbf} and
\ics{hyperemph} are primary records. In this case, the bold format
is used to indicate the primary location in the main document text
and the emphasized format is used to indicate the location in the
main glossary.
...
The main glossary records are added through the category-independent
post-name hook with \cs{glsadd}. This won't be implemented until the
entries are actually defined as the actual page number won't be
known until the glossary can be displayed. This means that the
document build requires an extra \bibgls\ and \LaTeX\ run:
\begin{verbatim}
pdflatex test
bib2gls --group test
pdflatex test
...
\begin{codeenv}
\gls{glsxtrnewglslike}\oarg{format=hyperbf,counter=chapter}\marg{}\marg{\csfmt{primary}}\marg{\csfmt{primarypl}}\marg{\csfmt{Primary}}\marg{\csfmt{Primarypl}}
\end{codeenv}

Here's another example that only has one primary format but uses
\ics{GlsXtrAutoAddOnFormat} to automatically insert
\ics{glsadd}\oarg{counter=chapter,format=hyperrm}\margm{label} on
each instance of \cs{gls} that has the format set to the custom
\code{primaryfmt}:
\begin{codeenv}
\csfmt{documentclass}\marg{report}
...
\gls!{GlsXtrLoadResources}\oarg{
  src=\marg{topics},
  primary-location-formats=\marg{hyperrm},
  save-primary-locations=\marg{remove}
}

\cs{newcommand}\marg{\csfmt{primaryfmt}}\oarg{1}\marg{\cs{hyperbf}\marg{\idx{param}1}}

\ics{GlsXtrAutoAddOnFormat}\marg{primaryfmt}\marg{counter=chapter,format=hyperrm}

\gls!{glsxtrnewglslike}\oarg{format=primaryfmt}\marg{}\marg{\csfmt{primary}}\marg{\csfmt{primarypl}}\marg{\csfmt{Primary}}\marg{\csfmt{Primarypl}}
...
\ics{printunsrtglossary*}\oarg{style=tree,title={Summary}}\marg{\comment{}
 \cs{renewcommand}*\marg{\ics{glsextrapostnamehook}}[1]\marg{\ics{glsadd}\oarg{format=hyperemph}\marg{\idx{param}1}}\comment{}
 \cs{renewcommand}\marg{\ics{GlsXtrLocationField}}\marg{primarylocations}\comment{}
}
...
Note that in this case, the \bibgls\ primary format is identified
as \code{hyperrm} not \code{primaryfmt}. This picks out the records
created with the automated \ics{glsadd}, which have the counter set
to \counter{chapter}. The first glossary (with the title
\qt{Summary}) switches the location field so that only the primary
records are listed. Since \styopt[nameref]{record} has been used
this means that the chapter title is shown rather than the chapter
number.
...
\begin{codeenv}
\gls{glsxtr@record}\margm{id}\marg{}\marg{wrglossary}\marg{glsnumberformat}\margm{n}
\end{codeenv}
...
\begin{codeenv}
\ics{GlsXtrSetField}\margm{id}\marg{indexcounter}\marg{\gls{glsxtr@wrglossarylocation}\margm{n}\margm{page}}
\end{codeenv}
...
\csref{glsxtrdisplaylocnameref} but the \meta{file} argument will now
be set.

As from \bibgls\ v1.7, any awkward characters if the file path are
replaced with \csref{bibglshrefchar} or (for non-ASCII characters when
\sty{fontspec} is loaded) \csref{bibglshrefunicode}. Both commands
take two arguments: the hexadecimal character code and the actual
character. In the case of \csref{bibglshrefchar}, the second
...
\end{codeenv}
In the case of \csref{bibglshrefunicode}, the first argument is
ignored.
...
\csref{GlsXtrLoadResources}. This includes the abbreviation style, 
