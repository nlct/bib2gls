...
\sty{glossaries-extra} will assume the encoding is \code{utf8} and
...
\cs{GlsXtrLoadResources}. Predefined fields for use in \ext{bib}
files are listed in Tables~\ref{tab:fields},
\ref{tab:bib2glsfields}, \ref{tab:prefixfields}
and~\ref{tab:accsuppfields}.  Fields that may be set by \bibgls\ are
listed in Table~\ref{tab:internalfields}. These typically shouldn't
be set in the \ext{bib} file and in some cases don't have a
designated key, in which case they won't be recognised if they're
present in the \ext{bib} file.
...
\fielddef{category} & This entry's category.\\
...
\fielddef{first} & The text to display on first use of \cs{gls}\\
\fielddef{firstplural} & The text to display on first use of \cs{glspl}\\
...
\fielddef{plural} & The text to display on subsequent use of \cs{glspl}\\
...
\fielddef{text} &  The text to display on subsequent use of \cs{gls}\\
...
\begin{table}[hbtp]
\caption{Fields Provided by \styfmt{glossaries-prefix}}\label{tab:prefixfields}
\medskip
\centering
\begin{tabular}{ll}
\bfseries Field & \bfseries Description\\
...
\begin{table}[hbtp]
\caption{Fields Provided by \styfmt{glossaries-accsupp}}
...
\begin{table}[hbtp]
\caption{Internal Fields}\label{tab:internalfields}
\centering
\medskip
\begin{tabular}{ll}
\bfseries Field & \bfseries Description\\
...
had selected.\\
\fielddef{counter} & The default counter used for indexing.\\
...
\fielddef{type} & The glossary this entry belongs to.
\end{tabular}
\end{table}
...
\optref[none:copies]{secondary} the \code{copies}
list will be ordered according to \code{de-CH-1996} and not
