...
However, with \entry{entry}, the fallback field will
...
add unrecorded entries whose \field{see} field refers to a recorded entry.
...
\csopt[\csref{bibglspagename},\meta{bibglspagesname}]{loc-prefix},
...
from the language resource file. The entries fetched from
...
\csopt[p.,pp.]{loc-prefix}. The entries fetched from
\texttt{entries1.bib} still won't have a location prefix, but the
entries fetched from both \texttt{entries2.bib} and
\texttt{entries3.bib} have the location prefixes obtained from the
...
value in the \ext{aux} file, but \bibgls\ will accept
a single space between \cs{u} and \meta{hex} to allow for this.
However it's safer to just use \cs{string}.
...
as \csopt[letter-nocase]{sort} as the letter ordering is rule-based
...
For example, if the sort value is \qt{sea lion} then it actually
...
You can change where the break points are insert with
...
marker to use. For example, \csopt[-]{break-marker} will use a
hyphen. The marker may be empty, which effectively strips the
inter-word punctuation. For example, with \csopt[{}]{break-marker}, 
\qt{sea lion} simply becomes \texttt{sealion}. If \meta{marker} is
omitted, \csopt[{}]{break-marker} is assumed.
...
title-casing for the digraph \qt{ij} when the main document language
is Dutch, in which case \meta{group} will be \qt{IJ} (rather than
\qt{Ij}).
