...
\section{Logical Divisions: \field{type} vs \field{parent} vs
\field{group}}
...
units for easier reading. There are three fields that can be used
for this.
\begin{description}
...
the order used to define the glossaries. You may use
...
ought to be using the \field{parent} field instead.
\end{important}

\item[\field{parent}] An entry may have one of more sub-entries.
...
the same comparator but taking the hierarchical level into account.
\end{important}
...
determined by the document's ordering of the \cs{printunsrtglossary}
...
Suppose that your list of symbols consists of Latin characters,
Greek characters and pictographs and you want them grouped together
...
\begin{codeenv}
\gls{glsxtrsetgrouptitle}\marg{latinsymbols}{Latin}
\gls{glsxtrsetgrouptitle}\marg{greeksymbols}{Greek}
\gls{glsxtrsetgrouptitle}\marg{pictographs}{Pictographs}
\gls{GlsXtrLoadResources}\oarg{
  \csopt[latinsymbols]{src},
  \csopt[latin]{group},
  \csopt[symbols]{type}
}
\gls{GlsXtrLoadResources}\oarg{
  \csopt[greeksymbols]{src},
  \csopt[greek]{group},
  \csopt[symbols]{type}
}
\gls{GlsXtrLoadResources}\oarg{
  \csopt[pictographs]{src},
...
default \code{main}, the \code{abbreviations} glossary created with
the \styopt{abbreviations} option and the \code{symbols} glossary
created with the \styopt{symbols} option). They are listed in the
order of \cs{printunsrtglossary} and their titles are added to the
table of contents.
...
have the same \field{group} value, this isn't a problem. The blocks
...
\strut
\gls{GlsXtrLoadResources}\oarg{\csopt[main]{type},\csopt[\marg{entrytype=entry}]{match}}
\gls{GlsXtrLoadResources}\oarg{\csopt[abbreviations]{type},\csopt[\marg{entrytype=abbreviation}]{match}}
\strut
\gls{glsxtrsetgrouptitle}\marg{pictograph}\marg{Pictographs}
\gls{GlsXtrLoadResources}\oarg{\csopt[symbols]{type},
 \csopt[\marg{identifier=\field{group}}]{field-aliases},
 \csopt[\marg{\field{group}=pictograph}]{match}}
\strut
\gls{glsxtrsetgrouptitle}\marg{latin}\marg{Latin Characters}
\gls{GlsXtrLoadResources}\oarg{\csopt[symbols]{type},
 \csopt[\marg{identifier=\field{group}}]{field-aliases},
 \csopt[\marg{\field{group}=latin}]{match}}
\strut
\gls{glsxtrsetgrouptitle}\marg{greek}\marg{Greek Characters}
\gls{GlsXtrLoadResources}\oarg{\csopt[symbols]{type},
 \csopt[\marg{identifier=\field{group}}]{field-aliases},
 \csopt[\marg{\field{group}=greek}]{match}}
\strut
\cmd{begin}\marg{document}
\cs{tableofcontents}
\cs{printunsrtglossary}\oarg{\printglossopt[abbreviations]{type}}
\cs{printunsrtglossary}
\cs{printunsrtglossary}\oarg{\printglossopt[symbols]{type},\printglossopt[treegroup]{style}}
\cmd{end}\marg{document}
\end{codeenv}
If this document is called \filefmt{myDoc.tex} then the document
build should be:
\begin{verbatim}
pdflatex myDoc
bib2gls myDoc
pdflatex myDoc
\end{verbatim}
Note that the \longarg{group} switch isn't used as there's no need
for \bibgls\ to form letter groups during sorting. If it is used
then the main and abbreviation glossaries will have groups formed.
With the default \glostyle{list} style this will just have vertical
gaps between the groups. You can switch those gaps off with the
\printglossopt{nogroupskip} option:
\begin{codeenv}
\cs{printunsrtglossary}\oarg{\printglossopt[abbreviations]{type},\printglossopt{nogroupskip}}
\cs{printunsrtglossary}\oarg{\printglossopt{nogroupskip}}
\end{codeenv}
In general it's simpler not to use the \longarg{group} switch if
it's not required, but if you have one glossary that needs it then
you can use this approach with the other glossaries. For example:
\begin{codeenv}
\cs{printunsrtglossary}\oarg{\printglossopt[abbreviations]{type},\printglossopt{nogroupskip}}
\cs{printunsrtglossary}\oarg{\printglossopt[indexgroup]{style}}
\end{codeenv}
...
\strut
\gls{GlsXtrLoadResources}\oarg{csopt[symbols]{type},
 \csopt[\marg{identifier=\field{parent}}]{field-aliases},
 \csopt[\marg{entrytype=symbol,entrytype=indexplural}]{match}}
\strut
\cmd{begin}\marg{document}
\cs{tableofcontents}
\cs{printunsrtglossary}\oarg{\printglossopt[abbreviations]{type}}
\cs{printunsrtglossary}
\cs{printunsrtglossary}\oarg{\printglossopt[symbols]{type},\printglossopt[topic]{style}}
\cmd{end}\marg{document}
\end{codeenv}
The document build is the same as before. The \longarg{group} switch
again isn't required. The style used for the symbols list is now
\glostyle{topic} rather than \glostyle{treegroup}. This results in a
slightly different appearance. You can select the most appropriate
style (see the
\href{https://www.dickimaw-books.com/gallery/glossaries-styles/}{gallery
of predefined styles}). The topic ordering is now: Greek characters,
Latin characters and Pictographs.
...
file. This will override \bibgls's normal group formation behaviour,
...
the \ext{bib} file (but note the above warning) or using the
\csopt[\meta{label}]{group} resource option. You will need to use
\gls{glsxtrsetgrouptitle} to provide an associated title if the
...
uses \atentry{comment} for metadata.
...
\printfields
 [%
  ...
  likely to be aliased). Don't define any of others listed in this
  ...
  that through a resource option. The explicit use of \field{sort}
  or \field{group} within a \ext{bib} file can cause unpredictable
  results and are best set through a resource option or by \bibgls.%
 ]%
 {internalfield}%
