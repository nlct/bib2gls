...
\csopt[Page\dcomma Pages]{loc-prefix}).
...
The required fields are \field{description} and either
...
pattern\footnote{The Java class \texttt{\csfmt{p}\marg{javaDigit}}
...
\subsection{\csoptnv{omit-alias-loc}=\margm{value}}
\labelopt{omit-alias-loc}

This is a boolean option. If \csopt[true]{omit-alias-loc}
the location lists for aliased entries will be omitted
(the cross-reference will be retained). If \meta{value} is omitted,
\optfmt{true} is assumed. The default setting
is \csopt[true]{omit-alias-loc}.
