...
If you already have a \ext{tex} file containing lots of 
entry definitions (\cs{newglossaryentry}, \cs{newacronym}, etc)
...
called \field{group}. If the \longarg{group} switch is used, When
...
example with the \abbrstyle{indexgroup} style). If you're not using
...
select an appropriate glossary style. If this switch is set, 
you can explicitly set the group field value for all entries using
the \optref{group} option.
...
\oarg{tag} part. If you need a different tag, either use \field{see}
...
The required fields are \field{name} or \field{parent}. If the
\field{sort} field is omitted, the default sort is given by the
entry label. Note that this is different from \entry{entry}
where the sort defaults to \field{name} or \field{parent}
if omitted. The \field{description} field is not required.

Terms defined using \atentry{symbol} will be written to
...
be copied to the secondary entry, nor can it be mapped. The alias will
only apply to the primary entry.

If the \field{sort} field is missing \bibgls\ by default falls back
on the \field{name} field. If this is missing, this entry type will
fallback on the \field{short} field.
...
If used with the \styopt{abbreviations} package option, this will
put the primary entry in the \optfmt{abbreviations} glossary and
the secondary entry in the \optfmt{main} glossary. 
Use the \optref{type} and \optref{dual-type} options to override
this.
...
\begin{definition}
\cs{bibglslettergroup}\margm{group}\margm{character}\margm{id}
\end{definition}
...
\meta{group}. The \meta{id} is a numeric identifier for that
group. (For a rule-based collation, this is obtained from the
first collation element. For a letter sort, it's
...
\meta{character} argument will be \texttt{\AA}.

The \meta{group} argument will typically by title-cased. An exception is made to the
title-casing for the digraph \qt{ij} which will be \qt{IJ} (rather than
\qt{Ij}).
...
This application recognises the following commands:
\begin{itemize}
\item \cs{newglossaryentry}\margm{label}\margm{key\dequals val list}
...
\cs{newentry} is recognised as a synonym of \cs{newglossaryentry}.

\item \cs{provideglossaryentry}\margm{label}\margm{key\dequals val list}

As above, but only if \meta{label} hasn't already been defined.

\item \cs{longnewglossaryentry}\margm{label}\margm{key\dequals val
list}\margm{description}
...
\item \cs{longprovideglossaryentry}\margm{label}\margm{key\dequals val
list}\margm{description}

As above, but only if \meta{label} hasn't already been defined.

\item \cs{newterm}\oarg{key\dequals val list}\margm{label}
...
\meta{key\dequals val list}. For example, if \texttt{entries.bib}
contains
\begin{verbatim}
\newterm{sea lion}
...
\item \cs{newabbreviation}\oarg{key\dequals val
list}\margm{label}\marg{short}\marg{long}
...
\item \cs{newacronym}\oarg{key\dequals val
list}\margm{label}\marg{short}\marg{long}

As above but uses \atentryref{acronym} instead.

\item \cs{glsxtrnewsymbol}\oarg{key\dequals val
list}\margm{label}\marg{symbol}
...
\end{alltt}
unless \meta{key\dequals val list} contains the \field{name} field,
...
\cs{newsym} is recognised as a synonym for \cs{glsxtrnewsymbol}.

\item \cs{glsxtrnewnumber}\oarg{key\dequals val
list}\margm{label}
...
\end{alltt}
if \field{name} isn't listed in \meta{key\dequals val list},
...
\cs{newnum} is recognised as a synonym for \cs{glsxtrnewnumber}.

\item \cs{newdualentry}\oarg{key\dequals val
list}\margm{label}\margm{short}\margm{long}\margm{description}

This command isn't provided by either \sty{glossaries} or
\sty{glossaries-extra} but is used in the sample file
...
\end{alltt}
if \meta{key=val list} is supplied, otherwise it's converted to:
\begin{alltt}
\atentryref{dualentryabbreviation}\marg{\meta{label},
  short = \margm{short},
  long = \margm{long},
...
In all cases, if \meta{key\dequals val list} contains
\begin{alltt}
