...
\begin{itemize}
\item \csopt{labelify} and \csopt{labelify-list} don't include
cross-reference fields (\field{see}, \field{seealso},
\field{alias} and \field{parent}), and
...
characters and replaces \qt{\O} (\hex{00D8}) with \qt{O} so the string now ends up as
\begin{verbatim}
...
space character \hex{A0} which isn't recognised by the date format.
This can easily be fixed with the \longarg{break-space} switch which
will interpret \verb|~| as a normal breakable space (\hex{20}), so 
