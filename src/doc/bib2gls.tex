...
\styfmt{glossaries-extra}'s \cs{printunsrtglossary}, which simply
iterates over all defined entries in the order in which they were defined
(or, more precisely, the order of the internal list of entry labels
associated with that glossary). The aim of \bibgls\ is to write the
entry definitions to the \ext{glstex} file so that the internal list
of labels is in the appropriate order.
...
\code{aardvark} (aardvark), \code{dual.css} (cascading stylesheets), 
\code{html} (HTML), \code{mouse} (mouse), 
\code{dual.ssi} (server-side includes), 
\code{xml} (XML), \code{zebra} (zebra).
The \code{abbreviations} glossary contains:
\code{css} (CSS), \code{dual.html} (HTML), \code{ssi} (SSI), 
\code{dual.xml} (XML).

Since all the entries were combined and sorted together, the
resulting glossaries are both ordered alphabetically, but note that
you need to take care when referencing the abbreviations if you want
to make use of the abbreviation style: \verb|\gls{css}| and
\verb|\gls{ssi}| for the primary abbreviations created with
\atentryref{dualabbreviationentry} and \verb|\gls{dual.html}|
and \verb|\gls{dual.xml}| for the dual abbreviations created with
\atentryref{dualindexabbreviation}.
...
\code{aardvark} (aardvark),
\code{css} (CSS),
\code{html} (HTML),
\code{mouse} (mouse),
\code{ssi} (SSI),
\code{xml} (XML),
\code{zebra} (zebra) and writes them to the \ext{glstex} file
(functionally like):
\begin{verbatim}
% from @index{aardvark}:
\newglossaryentry{aardvark}{name={aardvark},description={}}
...
\code{dual.css} (cascading stylesheets),
\code{dual.html} (HTML),
\code{dual.ssi} (server-side includes),
\code{dual.xml} (XML)
...
\begin{enumerate}
\item \code{aardvark} (assigned to \code{main}),
\item \code{css} (assigned to \code{abbreviations}),
\item \code{html} (assigned to \code{main}),
\item \code{mouse} (assigned to \code{main}),
\item \code{ssi} (assigned to \code{abbreviations}),
\item \code{xml} (assigned to \code{main}),
\item \code{zebra} (assigned to \code{main}),
\item \code{dual.css} (assigned to \code{main}),
\item \code{dual.html} (assigned to \code{abbreviations}),
\item \code{dual.ssi} (assigned to \code{main}),
\item \code{dual.xml} (assigned to \code{abbreviations})
\end{enumerate}
This means that the \code{main} glossary's internal list is in the
order: \code{aardvark} (aardvark), \code{html} (HTML), \code{mouse}
(mouse), \code{xml} (XML), \code{zebra} (zebra), 
\code{dual.css} (cascading stylesheets), 
\code{dual.ssi} (server-side includes), and the
\code{abbreviations} glossary's internal list is in the order:
\code{css} (CSS), \code{ssi} (SSI), \code{dual.html} (HTML), 
\code{dual.xml} (XML).
...
description). A better solution would be to use 
...
This entry type is deprecated as from version 1.1. It's functionally
equivalent to \atentryref{dualabbreviationentry}, 
but the primary is defined using
\csref{bibglsnewdualentryabbreviation} and the dual is defined
using \csref{bibglsnewdualentryabbreviationsecondary}.
...
The \atentry{tertiaryindexabbreviationentry} entry type is
very similar to \atentryref{dualindexabbreviation} but creates a
tertiary entry as well. The mappings are shared by both entry types.
...
opts}\margm{name}\margm{short}\margm{long}\margm{description}
\end{definition}
...
entry.
Note that the \field{name} uses \plabelcs{bibglsuselongfont}
\begin{definition}
