...
\end{codeenv}
where \meta{field-list} is a comma-separated list of valid fields, 
\meta{sort} is a valid sort method (as per the \csopt{sort} option)
and \meta{csname} is the name (without a leading backslash) of a
command that takes a label as its sole mandatory argument that's
recognised by \bibgls' interpreter (such as those listed in
\tableref{tab:bibglsdefs}). If the list has more than one element
...
After the main sorting of each set of selected entries is performed, if this
