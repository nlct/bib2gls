...
The \exfile{people.bib} file contains details about people. This
starts with a definition of \gls{sortname} and \gls{sortvonname} 
in \atentry{preamble} that can be picked up by the interpreter. This
swaps the order of the arguments which allows \bibgls\ to sort by
surname. Remember that if \exfile{no-interpret-preamble.bib} is
...
provided by the \sty{glossary-bookindex} package. This isn't loaded
...
This example uses the \exfile{no-interpret-preamble.bib} and
\exfile{people.bib} files. The aim here is to have a list of people
ordered alphabetically by surname with a brief description, 
the same list ordered by date of birth and an index of all the
people without their details and with a number list indicating where
the people were mentioned in the document. The first two lists
shouldn't include aliases but the index should.
...
\sty{glossary-bookindex},\footnote{\styfmt{glossary-bookindex}
is distributed with \styfmt{glossaries-extra} v1.21+} so I need:
\begin{verbatim}
