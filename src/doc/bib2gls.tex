...
then you can use the supplementary tool \app{convertgls2bib}
...
(or the more convenient shortcut \cs{GlsXtrLoadResources}).  This
means the entries are already defined in the correct order, and only
those entries that have been used in the document are defined, so
\cs{printunsrtglossary} (or \cs{printunsrtglossaries}) may be used.
(The \texttt{unsrt} part of the command name indicates that all
...
file}, where \meta{lang} is a valid \gls{IETF} language tag.
...
by the following line in the \hyperref[sec:lang.xml]{language resource file}:
\begin{verbatim}
...
\atentry{entry} where the sort defaults to \field{name} or
\field{parent} if omitted. (You can use \optref{sort-field} to
...
The \atentry{index} entry type is designed for entries that don't have
...
example \cs{newdualentry} command used in the
\texttt{sample-dual.tex} file provided with the \sty{glossaries}
package. The primary entry is an abbreviation with the given
...
\keyvallist. Allowed options are listed below.
...
\subsection{\csoptnv{secondary-sort-rule}=\margm{value}}
...
long-winded detour.
\subsection{\csoptnv{match}=\margm{\keyvallist}}
\labelopt{match}
...
\subsection{\csoptnv{flatten-lonely}=\margm{value}}
\labelopt{flatten-value}

This may take one of three values: \optfmt{false} (default),
\optfmt{presort} and \optfmt{postsort}. The value must be supplied. 

If not \optfmt{false}, this option will check for sub-entries that
have no siblings selected and have a parent with no records. If
found, the parent entry will be removed. The entry's \field{name}
and \field{text} fields may be adjusted in the process. The parent
will either be removed before sorting
(\csopt[presort]{flatten-lonely}) or after sorting
...
\item[minerals] \mbox{}
  ...
  \end{description}
\end{description}
The \texttt{duck} entry looks a little odd, and it would be better
if its hierarchical structure was removed. In this case, I don't
want to use the \optref{flatten} option, as I still want to keep the
mineral hierarchy.

The \csopt[presort]{flatten-lonely} setting will remove the parent from the
\texttt{duck} entry \emph{before} sorting. Since no \field{text} field has
been supplied, it will be set to the original \field{name} (which,
in this case, falls back to the label). The \field{name} field is
then set to
\begin{verbatim}
\bibglsflattenedchildpresort{duck}{birds}
\end{verbatim}
which defaults to just \qt{duck}. The \field{parent} field is
removed and the parent entry (\texttt{birds}) is discarded from the
selection.
This means it behaves (roughly) as though \texttt{duck} was defined as
\begin{verbatim}
@index{duck,
...
\end{verbatim}
The interpreter recognises \csref{bibglsflattenedchildpresort} and
determines that the sort value is just \qt{duck}. The entries are
then sorted and the resulting hierarchy is now:
\begin{description}\setlength\itemsep{0pt}%
\item[A] \null
\item[aardvark] 1
\item[B] \null
...
\item[minerals] \mbox{}
  ...
  \end{description}
\end{description}
Note that the \qt{duck} entry has now moved to the \qt{D} letter
group.

If, on the other hand, I use \csopt[postsort]{flatten-lonely}, the
parent entry (\texttt{birds}) is discarded \emph{after} sorting. The
\field{text} field for \texttt{duck} is again set to \qt{duck} but
now the \field{name} field is set to
\begin{verbatim}
\bibglsflattenedchildpostsort{birds}{duck}
\end{verbatim}
which defaults to \qt{birds, duck}. Since the parent entry is
discarded after sorting, the \texttt{duck} entry is in the
same relative location as for \csopt[false]{flatten-lonely}, but
it's moved up a hierarchical level, so the listing is now:
\begin{description}\setlength\itemsep{0pt}%
\item[A] \null
\item[aardvark] 1
\item[B] \null
...
\item[minerals] \mbox{}
  ...
  \end{description}
\end{description}
Since the \field{text} field has been set to \texttt{duck},
\verb|\gls{duck}| still produces just \qt{duck}.
...
\field{name} field is typically set by the abbreviation style.
Remember that the parent entry is discarded after all relevant
entries and their dependencies have been selected, so any
cross-references within the parent entry may end up being selected
even if they wouldn't be selected if the parent entry didn't exist.
The parent entry won't be discarded if it contains any records. This
includes ignored records (identified with \gencsopt[glsignore]{glsadd}{format}).
...
\subsection{\csoptnv{group}=\margm{value}}
\labelopt{group}
...
\subsection{\csoptnv{save-child-count}=\margm{boolean}}
...
The \meta{format} is that supplied by the \gencsopt{gls}{format} key when using
commands like \cs{gls} or \cs{glsadd}. If omitted,
...
\optfmt{glsignore} is used in a range
(for example,
\texttt{\csfmt{glsadd}[format\dequals(glsignore]\marg{sample}} 
\ldots\ 
\texttt{\csfmt{glsadd}[format\dequals)glsignore]\marg{sample}}) 
...
\subsection{\csoptnv{seealso}=\margm{value}}
\labelopt{seealso}
...
\subsection{\csoptnv{sort-rule}=\margm{value}}
\labelopt{sort-rule}
...
\subsection{\csoptnv{break-at}=\margm{option}}
\labelopt{break-at}
...
\subsection{\csoptnv{break-marker}=\margm{marker}}
...
\subsection{\csoptnv{shuffle}=\margm{seed}}
\labelopt{shuffle}
...
\subsection{\csoptnv{strength}=\margm{value}}
\labelopt{strength}
...
\subsection{\csoptnv{decomposition}=\margm{value}}
...
\subsection{\csoptnv{dual-sort-rule}=\margm{value}}
...
\subsection{\csoptnv{dual-entryabbrv-backlink}=\margm{boolean}}
...
\section{\cs{bibglsnewdualentryabbreviation}}
...
\atentry{dualentryabbreviation} type. The definition provided in the \ext{glstex}
file is:
\begin{verbatim}
...
\section{\cs{bibglsnewdualentryabbreviationsecondary}}
...
The default definition is obtained from the \hyperref[sec:lang.xml]{language resource
...
For example, suppose on page~4 I start a range with
\begin{verbatim}
...
using the value of \texttt{tag.page} from the \hyperref[sec:lang.xml]{language resource
file}. For example with \file{bib2gls-en.xml} the definition is:
\begin{verbatim}
\providecommand{\bibglspagename}{Page}
\end{verbatim}

\section{\cs{bibglspagesname}}
...
If \csopt[true]{loc-prefix} is used, then this command is provided
using the value of \texttt{tag.pages} from the \hyperref[sec:lang.xml]{language resource
file}. For example with \file{bib2gls-en.xml} the definition is:
\begin{verbatim}
...
will set the \field{group} field to contain this command. The
\meta{title} argument indicates the letter group and the
\meta{letter} argument indicates the actual initial letter of
the sort value, which may not be the same as \meta{title}. The
\meta{id} is a numeric identifier for that group. (For a rule-based
collation, this is obtained from the first collation element. For a
letter sort, it's obtained from the character code.) The \meta{type}
indicates the associated glossary type. This value will be empty if
unknown.
...
The \meta{title} argument will typically by title-cased. An
exception is made to the title-casing if the
\hyperref[sec:lang.xml]{language resource file} contains the key
\texttt{grouptitle.case.\meta{lc}} where \meta{lc} is the lower case
version of \meta{title}. See the \argref{group} switch
for more details.

The default definition of this command depends on the
existence of \cs{glsxtrsetgrouptitle}, which was introduced
to \sty{glossaries-extra} v1.14.

\section{\cs{bibglsothergroup}}
\labelcs{bibglsothergroup}

\begin{definition}
\cs{bibglsothergroup}\margm{character}\margm{codepoint}\margm{type}
\end{definition}
This command is provided if the \argref{group} switch is used. Any
...
character. (Note that the \meta{type} is empty as the \field{type}
field is missing and the \optref{type} option hasn't been used.)

The default definition of this command is:
\begin{verbatim}
\providecommand{\bibglsothergroup}[3]{\glssymbolsgroupname}
\end{verbatim}
This ignores all arguments but could be redefined to sub-divide the
...
\begin{definition}
\cs{bibglsnumbergroup}\margm{number}\margm{type}
\end{definition}
...
The default definition of this command is:
\begin{verbatim}
\providecommand{\bibglsnumbergroup}[2]{\glsnumbersgroupname}
\end{verbatim}

\section{\cs{bibglshypergroup}}
...
\section{\cs{bibglsflattenedchildpresort}}
...
\section{\cs{bibglsflattenedchildpostsort}}
...
then this can confuse the case-changing mechanism.
