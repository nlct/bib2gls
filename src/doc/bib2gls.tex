...
following information for each entry based on the first letter of
...
\subsection{\csfmt{bibglssetgrouptitle}}
\labelcs{bibglssetgrouptitle}

For each group that's detected, \bibgls\ will write the line:
\begin{definition}
\cs{bibglssetgrouptitle}\marg{\margm{title}\margm{letter}\margm{id}\margm{type}}
\end{definition}
...
Note that \cs{bibglssetgrouptitle} only has a single
...
version 1.14 onwards), then \cs{bibglssetgrouptitle} will be
defined as
\begin{verbatim}
\providecommand{\bibglssetgrouptitle}[1]{%
...
Since \cs{bibglssetgrouptitle} is used in the \ext{glstex} file to
...
\begin{alltt}
\cs{bibglssetgrouptitle}\marg{\marg{\AE}\marg{\ae}\marg{7274496}\marg{}}
\end{alltt}
...
\GlsXtrLoadResources[
  ...
  dual-type=english,% put the primary entries in the 'english'
glossary
  dual-prefix={en.}
]
\end{verbatim}
then CJK characters will appear in the \meta{title} argument of 
\cs{bibglslettergrouptitle} which causes a problem. This needs
encapsulating with the \env{CJK} environment, which can be more
...
\end{definition}
Any entries whose \field{sort} value indicates that they are not in a letter
