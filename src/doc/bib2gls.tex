...
\item if the \field{parent}, \field{category}, \field{type} 
or \field{alias} fields contain
...
\styfmt{glossaries-extra-stylemods}), but this requires knowing which
name is the widest.
...
using \gls{bibglssetwidest} if the \field{type} is unknown
or \gls{bibglssetwidestfortype}.  This leaves \TeX\ to
...
full stop, this will be considered an end of sentence terminator, but
...
The \atentry{symbol} and \atentry{number} entry types will, by default,
fallback on the label if the \field{sort} field is
missing (assuming the default \csopt[sort]{sort-field}). If you prefer to
fallback on a different field, then you can use this option to
specify the field. For example, \csopt[name]{symbol-sort-fallback}.
The \meta{field} value must be a known field label.
...
fields all use \ics{si} provided by the \isty{siunitx} package, so
...
relational symbols. These use the maths shift character \verb|$| in
the \field{name} field and just the symbol in the \field{text}
field. This just illustrates an alternative way of defining symbols.
Since \ics{ensuremath} isn't used, commands \cs{gls} must be
explicitly placed in maths mode. For example, \verb|$\gls{leq}$|
rather than simply \verb|\gls{leq}|. The custom \fieldfmt{identifier}
field is set to \code{relation}.
...
\nosecdef{setmembershipineargfmt}
...
\code{set} for sets, \code{vector} for vectors and \code{matrix} for
matrices.
...
\exfile{no-interpret-preamble} and \exfile{interpret-preamble} are
also needed.
...
stop, so I've used the \styopt{postdot} package option. 
...
find the widest \field{name}, which can be done with
\csopt{set-widest}.
...
This example uses 
\exfile{mathsrelations.bib}, \exfile{binaryoperators.bib},
\exfile{unaryoperators.bib} and \exfile{mathsobjects.bib}.

This file has a fairly similar preamble to \exfile{sample-msymbols},
but \exfile{no-interpret-preamble} is now needed to provide
the \csref{sortart} command for use in the document:
\begin{verbatim}
\GlsXtrLoadResources[
   src={no-interpret-preamble},
   interpret-preamble=false
...
indexed, so I've omitted the \csref{selection} option, however
I still don't want any number lists so I still have
\csopt[false]{save-locations}.

To make it easier to use \csref{glsxtrfmt}, I've provided some
semantic commands. These actually use the starred form
\csref{glsxtrfmt*} to make use of the optional argument, but
remember that this requires \sty{glossaries-extra} version 1.23+.
I want \csref{glsxtrfmt} to index which means I need to redefine
\ics{GlsXtrFmtDefaultOptions} so that it doesn't use \code{noindex}:
\begin{verbatim}
\renewcommand{\GlsXtrFmtDefaultOptions}{}
\end{verbatim}
I've provided some convenient wrapper commands that use
\csref{glsxtrfmt*} or the non-linking \csref{glsxtrentryfmt}:
\begin{verbatim}
...
\end{verbatim}
The use of this starred form allows:
\begin{verbatim}
