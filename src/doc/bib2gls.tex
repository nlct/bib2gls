...
\end{codeenv}
The \exfile{chemicalformula.bib} file only has custom fields, so these 
...
command that instructs \bibgls\ to make \atentry{symbol} behave like
\atentry{dualsymbol}:
\begin{codeenv}
...
natural word ordering to use a locale sort method while the
...
\fieldfmt{unitsymbol} field. This means that I need to set a label
prefix to prevent duplicate labels:
\begin{codeenv}
\gls{GlsXtrLoadResources}\oarg{
 \csopt[baseunits]{src},
 \csopt[
 ...
 \csopt[baseunit]{category},
 \csopt[baseunit]{type},
 \csopt[baseunit.]{label-prefix},
 \csopt[baseunit.]{record-label-prefix},
 \csopt{set-widest},
 \csopt[name]{symbol-sort-fallback},
 \csopt[false]{save-locations}
}
...
index, but first \ics{glsxtrlongshortdescname} needs to be changed
...
\csopt[long]{abbreviation-sort-fallback}
\end{codeenv}
The symbols need to fallback on the \field{name} field:
\begin{codeenv}
\csopt[name]{symbol-sort-fallback}
\end{codeenv}
The custom fields again need to be aliased:
\begin{codeenv}
\csopt[
   identifier=category,
   formula=symbol,
...
\end{codeenv}
Again it's necessary to provide label prefixes to avoid conflict:
\begin{codeenv}
\csopt[idx.]{label-prefix},
...
\csopt[category]{identical-sort-action}
\end{codeenv}
These entries should all be in the index:
\begin{codeenv}
\csopt[index]{type}
\end{codeenv}
...
so here I've done the reverse to earlier and copied the
...
index and not in the list of people.
...
\ics{glsxtrnewgls}\marg{baseunit.}\marg{\csfmt{baseunit}}
\cs{glsxtrnewgls}\marg{film.}\marg{\csfmt{film}}
\cs{glsxtrnewglslike}\marg{idx.}\marg{\csfmt{idx}}{\csfmt{idxpl}}{\csfmt{Idx}}{\csfmt{Idxpl}}
\end{codeenv}
So a term that only appears in the index is referenced with
\csfmt{idx} (or \csfmt{idxpl}, \csfmt{Idx}, \csfmt{Idxpl}).
...
\begin{figure}
\figcontents
{\pdftwocol[9]{../examples/sample-multi2.pdf}{10}}
{\caption{\filefmt{sample-multi2.pdf} (pages~9 and 10)}}
