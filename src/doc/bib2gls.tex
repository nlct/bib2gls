...
A custom handler function is provided for the main glossary that
...
\cs{newcommand}\marg{\csfmt{nestedhandler}}[1]\marg{\comment{}
  ...
  \cs{ifglossaryexists}*\marg{#1}\comment{}
  \marg{\comment{}
    \cs{printunsrtinnerglossary}\oarg{\printglossopt[\idx{hashchar}1]{type},\printglossopt[++1]{leveloffset},\printglossopt[false]{groups}}\marg{}\marg{}\comment{}
  }\comment{}
  \marg{}\comment{}
}
\end{codeenv}
...
\begin{codeenv}
\cs{printunsrtglossary*}\marg{\comment{}
  \cs{let}\cs{printunsrtglossaryhandler}\csfmt{nestedhandler}
}
\end{codeenv}
...
One way to ensure that the \qt{animal}, \qt{mineral} and
\qt{vegetable} entries are selected is to identify the \field{type}
...
hierarchical levels. The sort value chosen for particular entries
can be altered through the use of fallbacks (such as the
\csopt{symbol-sort-fallback} option) and a letter comparator may be
used to resolve identical sort values, but the same method is
applied to all entries in the same set. The only way to apply
different sort methods is to separate the entries into different
glossaries.
...
\gls{GlsXtrLoadResources}\oarg{\csopt[terms,animals,minerals,vegetables]{src},
 ...
 \csopt[{identifier=parent}]{field-aliases},
 \csopt[deps but not recorded]{selection}
}
...
permitted so the simpler alternative works.
