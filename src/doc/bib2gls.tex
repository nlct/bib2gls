...
\idx{mshiftchar}\meta{maths}\idx{mshiftchar}, \code{\ics{ensuremath}\margm{maths}} 
and \code{\ics{si}\margm{text}});
...
\begin{enumerate}
\item if \ics{protect} then skip and move on to the next token;
\item if \idx{mshiftchar}\meta{maths}\idx{mshiftchar} or \ics{NoCaseChange}\margm{text} or
\code{\ics{ensuremath}\margm{maths}} or \code{\ics{si}\margm{text}} then stop (no case change);
...
\meta{text} (using the same set of rules) and stop;
