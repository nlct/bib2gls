...
\item \optfmt{date}: sort dates (see below). Use \optref{date-sort-format}
...
\item \optfmt{datetime}: sort date and time information. Use \optref{date-sort-format}
and \optref{date-sort-locale} to specify the date-time format and locale.

\item \optfmt{datetime-reverse}: as above but reverses the order.

\item \optfmt{time}: sort times. Use \optref{date-sort-format}
and \optref{date-sort-locale} to specify the time format and locale.
...
be set with \optref{sort-date-locale}. The pattern is assumed
to be the default for that locale but you can change this with
\optref{sort-date-format}.

The locale and pattern information is used by \bibgls\ to
parse the field. The \field{sort} field is then set to the ISO
format, but the sort comparator actually uses an integer
representation of the date rather than performing a letter sort.
This means that if you need the more convenient ISO format in your
document you can fetch it from the \field{sort} field.

If the field value can't be parsed then \bibgls\ will issue a
warning and assume the current date (or time).
...
The \field{sort} field is then set to the ISO form 2017-01-10,
which makes it much easier to compare. In general, it doesn't
make much sense to have hierarchical entries that need to 
be sorted by date, but it is possible as long as each level
...
\begin{verbatim}
20170110 -> '2017-01-10' [1484006400000]
\end{verbatim}
The first value is the label (\code{20170110}), the second
value is the ISO date assigned to the \field{sort} field
(\code{2017-01-10}) and the number in square brackets 
is the actual numeric value used by the comparator.
This value is the number of milliseconds since 1st~January, 1970.
...
\begin{verbatim}
\providecommand{\bibglsdatetimegroup}[9]{#1#2\@firstofone}
\end{verbatim}
This forms the group label from the year, month and \meta{type}.
...
The default definition is
\begin{verbatim}
\providecommand{\bibglsdatetimegrouptitle}[9]{#1-#2\@gobble}
\end{verbatim}
This sets the title to the numeric \code{\meta{YYYY}-\meta{MM}}
...
\begin{definition}
\cs{bibglsdategroup}\margm{YYYY}\margm{MM}\margm{DD}\margm{title}\margm{group id}\margm{type}
\end{definition}

This command is used for group labels with date sorting (for
example, with \optref[date]{sort}).

\subsection{\csfmt{bibglsdategrouptitle}}
\labelcs{bibglsdategrouptitle}

\begin{definition}
\cs{bibglsdategrouptitle}\margm{YYYY}\margm{MM}\margm{DD}\margm{title}\margm{group id}\margm{type}
\end{definition}
