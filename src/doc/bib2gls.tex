...
Unlike the \optref{flatten} option (describe above) which completely
...
\item if \csopt[only unrecorded parents]{flatten-lonely-rule}
      ...
      cross-references (otherwise the parent may have a location
...
the sorting is performed. For example, with
\begin{verbatim}
\GlsXtrLoadResources[src={entries.bib},
 flatten-lonely-rule=discard unrecorded,
...
\subsection{\csoptnv{flatten-lonely-rule}=\margm{value}}
...
that have a parent without a location list will be selected.
