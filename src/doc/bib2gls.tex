...
or \field{alias} fields contain \idx{escchar}
...
(such as \ics{MakeUppercase}) and won't expand other commands
...
or \ics{cs.hash}, \ics{cs.percent}, \ics{underscore}, \ics{cs.amp},
\ics{cs.openbrace} and \ics{cs.closebrace} to indicate
\idx{hashchar}, \idx{percentchar}, \idx{underscorechar},
\idx{ampchar}, \idx{openbracechar} and \idx{closebracechar}.
\end{itemize}
...
\optsection[\subsubsection]{secondary-sort-number-pad}

As \csopt{sort-number-pad} but for secondary entries.

\optsection[\subsubsection]{secondary-sort-pad-plus}

As \csopt{sort-pad-plus} but for secondary entries.

\optsection[\subsubsection]{secondary-sort-pad-minus}
...
All entries are defined using \atentry{symbol}. With this entry
type, if the \field{sort} field is omitted \bibgls\ will fallback on
the label by default. It can be changed to fallback on the
\field{name} field instead using \csopt[name]{symbol-sort-fallback}. 
This will require the use of the interpreter as
...
Note that I've used \ics{glshyperlink} rather than \ics{gls} in the
\field{description} fields. This prevents the referenced entry from
being indexed, but \bibgls\ will pick up the cross-reference and
(with the default \csopt{selection}) will added the dependent
entries. This means that the dependent entries will be included but
won't have a number list if they haven't been referenced in the
document.
...
\end{codeenv}
The entries are all defined using \atentry{symbol} but I've decided
...
\begin{codeenv}
\csopt[list,bookindex]{stylemods}
\end{codeenv}
...
been indexed and any cross-references. The cross-references that
haven't been index won't have a number list (since they're
referenced with the non-indexing \ics{glshyperlink}). I've provided
...
\filesection{sample-multi.tex}

This example uses \exfile{bacteria.bib},
\exfile{markuplanguages.bib}, \exfile{minerals.bib}.
...
(without number lists) for each type of data: bacteria, and also an
...
\csopt[recorded and deps and see]{selection} to ensure the aliased
...
glossary so I've used the \styopt{index} package option. The
remaining glossaries need defining:
\begin{verbatim}
...
make \bibgls\ treat them as though they were:
\begin{codeenv}
\csopt[{
  \marg{abbreviation=dualindexabbreviation},
  \marg{entry=dualindexentry}
 }]{entry-type-aliases}
\end{codeenv}
...
\csopt[long]{abbreviation-name-fallback}
\end{codeenv}
Remember that the sort fallback is still \field{short} (but can be
changed with \csopt{abbreviation-sort-fallback}).
...
custom \field{identifier} field, I can both alias this custom field
...
\emph{Clostridium tetani} in the index.

I'd like to sort the dual entries according to a letter-number rule,
as for \exfile{sample-chemical.tex} and \exfile{sample-units3.tex},
...
\end{codeenv}
The default \csopt[sort]{dual-sort-field} is unchanged, which means
that with the default \csopt[short]{abbreviation-sort-fallback}, 
...
The complete document code is listed below. The document build is:
\begin{verbatim}
pdflatex sample-multi
bib2gls --group sample-multi
pdflatex sample-multi
\end{verbatim}
The resulting document is shown in \figureref{fig:sample-multi.pdf}.

\lstinputlisting[firstline=5]{../examples/sample-multi.tex}

\begin{figure}[p]
\figcontents
{%
 \frame{\includegraphics[height=.9\textheight]{../examples/sample-multi.pdf}}%
}
{\caption{\filefmt{sample-multi.pdf}}}
{fig:sample-multi.pdf}
\end{figure}
