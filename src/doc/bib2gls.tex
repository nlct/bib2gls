...
\gls{GlsXtrLoadResources} represents a \idx{resourceset}. Each
...
the \idx{resourceset} are parsed. Preamble information (provided by
...
\item[Stage 3 (Processing Entries)] For each entry that was found in the
corresponding set of \ext{bib} files: required field assignments are made
(for example, \csopt{field-aliases} and \csopt{replicate-fields}),
any fields that may only represent labels are interpreted (if
\csopt{interpret-label-fields} is set), dependencies checked (if
\csopt[recorded and deps]{selection} or \csopt[recorded and deps and
see]{selection}) and the dual version (if appropriate) is created.
Any entries that should be discarded are filtered from the list.
The transcript will show the message
\begin{alltt}
...
performed (if required) and the \iext{glstex} file is written.
The transcript will show the message
\begin{alltt}
...
\end{verbatim}
The mode can only be enabled if the following criteria is satisfied:
\begin{itemize}
\item the interpreter is off, or
\item every \idx{resourceset} either doesn't have a preamble or has
...
entry $A$ has been indexed and depends on entry $B$ that hasn't been
indexed, then $B$ can be picked up from a different
...
been indexed because $B$ hasn't been indexed and is in a different
\idx{resourceset}.
...
files. Note that the \field{see} field don't use the interpreter as
...
\csopt[false]{strip-missing-parents} is on, this only occurs
while writing the definitions in the \ext{glstex} file.
Sorting is performed hierarchically and the \field{group} is set
accordingly for the top-level entries, which means that an entry
with a \field{parent} field will be treated by the sort method
as a child entry. This can lead to a strange result, which \bibgls\
warns about:
\begin{alltt}
...
\bibgls\ unless defined or aliased: \fieldfmt{identifier} set to
\code{chemical} for all entries, \fieldfmt{formula} set to the
chemical formula, and \fieldfmt{chemicalname} set to the chemical
...
The \exfile{baseunits.bib} file contains base \glspl{SIunit}. The entries
are all defined using \atentry{symbol} but have custom fields
\fieldfmt{unitname}, \fieldfmt{unitsymbol} and \field{measurement},
one of which must be aliased or copied to \field{name}. The others
may be aliased or copied to \field{symbol} and \field{description}
as required. The \field{unitsymbol} fields all use \ics{si} provided
by the \isty{siunitx} package, so that package must be loaded in the
...
required. Remember that \atentry{symbol} entry types use the label
as the fallback field for \field{sort}. In this case the labels all
happen to match the \field{name}.
...
nor is this data suitable for sorting according to the \field{name}
field. Instead, either sort by label (which is the default for
\atentry{symbol}) or by the \field{description}. If you want to use
one of the \glostyle{alttree} styles you can still use
\csopt{set-widest}, but it will have to use the fallback command.
Alternatively, you can omit \csopt{set-widest} and explicitly use
...
\end{codeenv}
This \ext{bib} file only has custom fields, so these need to be
aliased. In this case I want the formula in the \field{name} field
...
\sty{glossary-mcols}. The entries are defined using
\atentry{symbol}, which uses the label as the default sort fallback,
so I've changed the fallback:
\begin{codeenv}
\csopt[name]{symbol-sort-fallback}
\end{codeenv}
The custom fields need to be aliased:
\begin{codeenv}
\csopt[
  unitname=name,
  unitsymbol=symbol,
...
As before the custom fields need to be aliased:
\begin{codeenv}
\csopt[
  unitname=name,
  unitsymbol=symbol,
...
\atentry{symbol} behave like \atentry{dualsymbol}:
\begin{codeenv}
\csopt[symbol=dualsymbol]{entry-type-aliases}
\end{codeenv}
This causes the \field{name} and \field{symbol} fields to be swapped
in the dual list. Remember that fallback for the \field{sort} field is the
label for the symbol entry types so I need \csopt[name]{sort-field}
to ensure that the sorting is by the \field{name} field instead.
...
\glspl{SIunit} are a mix of letters and numbers I've used one of the
...
\csopt[true]{interpret-field-labels}.  This means that with this
...
recorded entries for the index. This allows the entries that have
natural word ordering to use a locale sort method while and the
entries that are symbolic can use one of the letter-number sort
methods.

This document uses some additional \ext{bib} files as it has some
extra glossaries, which all need to be defined:
\begin{verbatim}
\newglossary*{bacteria}{Bacteria}
\newglossary*{markuplanguage}{Markup Languages}
\newglossary*{vegetable}{Vegetables}
...
and \exfile{films.bib} it also requires
\exfile{no-interpret-preamble.bib} and either
\exfile{interpret-preamble.bib} or \exfile{interpret-preamble2.bib}
to ensure the custom commands are provided.
...
prevents them from being added to the interpreter. The
\exfile{people.bib} file is the next to be loaded with
...
this setting doesn't affect the other entries:
\begin{codeenv}
\gls{GlsXtrLoadResources}\oarg{
...
\csopt[people]{category}, this obtains the \field{category} field
...
\glssetcategoryattribute{markuplanguage}{glossdesc}{firstuc}
\end{verbatim}
Next come the entries with symbolic names, which require a
letter-number sort:
\begin{codeenv}
\gls{GlsXtrLoadResources}\oarg{
 \csopt[chemicalformula,baseunits,derivedunits]{src},
 \csopt[
   identifier=category,
   formula=name,
 ...
 \csopt[same as category]{type},
 \csopt{set-widest},
 \csopt[letternumber-upperlower]{sort},
 \csopt[name]{symbol-sort-fallback},
 \csopt[false]{save-locations}
}
\end{codeenv}
