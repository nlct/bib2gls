...
option, but not including \optfmt{none}, \optfmt{unsrt} or
\optfmt{combine};
...
\csopt[combine]{dual} then there is only one list that contains both
...
steps are performed:
\begin{enumerate}
\item For each entry with the label \meta{id}:
 \begin{enumerate}
  ...
  \begin{enumerate}
   \item The field value must be in the form
     ...
     (optionally preceded by \oargm{tag});
   \item The \meta{label-list} is converted into a list where the
    $i$th element is
    \code{\marg{\cs{csuse}\margm{csname}\margm{label$_i$}}}
    unless \meta{csname} hasn't been set, in which case the
    $i$th element is just \meta{label$_i$} (the optional
    \oargm{tag} part is omitted);
   \item This new list is sorted according to the interpreter's
    definition of the command given \meta{csname} and the designated
    sort method;
 ...
 \end{enumerate}
\end{enumerate}
Note that there is no hierarchy structure in the sorting of the field list
even if any of the referenced entries has a parent.

For example, if the file \filefmt{entries.bib} contains:
\begin{codeenv}
\atentry{index}{bird}
\strut
\atentry{index}\marg{waterfowl,
...
\strut
\atentry{index}\marg{tardigrade,
  \field{name}=\marg{water bear}
}
\strut
...
\cmd{end}\marg{document}
\end{codeenv}
This reorders the \field{see} and \field{seealso} fields according to
the referenced entry's name.
...
\field{see}=\marg{[related terms]fluffy,velociraptor,duck,tardigrade}
\end{codeenv}
but in the \filefmt{glstex} file it's written as:
\begin{codeenv}
...
bear}. If no encapsulating command was specified:
\begin{codeenv}
...
\end{codeenv}
In this case \code{\ics{glsxtrhiername}\marg{swan}} is converted to
\qt{bird, waterfowl, swan} so the \field{seealso} field for the
\code{duck} entry ends up as:
\begin{codeenv}
...
to use \ics{glsxtrhiername}. The separator between the levels is
given by \ics{glsxtrhiernamesep} which defaults to a comma followed
by a space. This can easily be confused with the cross-reference
list comma, so you may want to redefine that as well. For example,
the following uses \ics{triangleright}:
\begin{codeenv}
\cmd{renewcommand}\marg{\cs{glsxtrhiernamesep}}\marg{ \idx{mshiftchar}\cs{triangleright}\idx{mshiftchar} }
...
document as: bird $\triangleright$ parrot, bird $\triangleright$
waterfowl $\triangleright$ goose, bird $\triangleright$ waterfowl
$\triangleright$ swan \& duckling.
...
The \filefmt{entries.bib} could contain:
\begin{codeenv}
...
the redefinition occurring in the document with \csopt[false]{write-preamble}.
...
comparators. However \optfmt{\meta{method}\dhyphen reverse}
