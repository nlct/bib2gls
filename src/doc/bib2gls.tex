...
input by \cs{GlsXtrLoadResources}. This extension is fixed and is
imposed by both \bibgls\ and \cs{GlsXtrLoadResources}. \bibgls\ also
...
\begin{codeenv}
\ics{glsxtr@record}\margm{label}\margm{prefix}\margm{counter}\margm{format}\margm{location}
\end{codeenv}
...
such as \cs{glsnoidxdisplayloc}, to allow for easy conversion to the
...
\ics{glsxtrnewglslike}\oarg{format=hyperbf}\marg{}\marg{\csfmt{primary}}\marg{\csfmt{primarypl}}\marg{\csfmt{Primary}}\marg{\csfmt{Primarypl}}
...
For consistency, I've used \cs{glsxtrnewglslike} to provide commands
...
\begin{codeenv}
\ics{glsxtrnewglslike}\oarg{format=hyperbf,counter=chapter}\marg{}\marg{\csfmt{primary}}\marg{\csfmt{primarypl}}\marg{\csfmt{Primary}}\marg{\csfmt{Primarypl}}
\end{codeenv}
...
\cs{glsxtrnewglslike}\oarg{format=primaryfmt}\marg{}\marg{\csfmt{primary}}\marg{\csfmt{primarypl}}\marg{\csfmt{Primary}}\marg{\csfmt{Primarypl}}
...
\begin{codeenv}
\cs{glsxtr@record}\margm{id}\marg{}\marg{wrglossary}\marg{glsnumberformat}\margm{n}
\end{codeenv}
...
\ics{glsxtrdisplaylocnameref} but the \meta{file} argument will now
be set.

As from \bibgls\ v1.7, any awkward characters if the file path are
replaced with \ics{bibglshrefchar} or (for non-ASCII characters when
\sty{fontspec} is loaded) \ics{bibglshrefunicode}. Both commands
take two arguments: the hexadecimal character code and the actual
character. In the case of \ics{bibglshrefchar}, the second
...
\begin{codeenv}
file\cs{bibglshrefchar}\marg{20}\marg{ }name.pdf
\end{codeenv}
In the case of \ics{bibglshrefunicode}, the first argument is
ignored.
