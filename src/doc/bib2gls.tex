...
\isty{graphicx}, \isty{ifthen}, \isty{jmlrutils}, \isty{mfirstuc},
...
(Note that \sty{mfirstuc} is automatically loaded by the base \sty{glossaries}
package, so it will always be present in the log file, but it's not
...
\begin{codeenv}
\meta{field} = \marg{\ics{MakeTextLowercase}\margm{text}}
\end{codeenv}
...
had actually been specified as:
\begin{codeenv}
\meta{field} = \marg{\ics{MakeTextUppercase}\margm{text}}
\end{codeenv}
...
had actually been specified as:
\begin{codeenv}
\meta{field} = \marg{\ics{makefirstuc}\margm{text}}
\end{codeenv}
...
had actually been specified as:
\begin{codeenv}
\meta{field} = \marg{\ics{capitalisewords}\margm{text}}
\end{codeenv}
...
\csopt{word-boundaries} setting. Exclusions (words that shouldn't be
changed) require the \TeX\ parser library's implementation of
\sty{mfirstuc}, which must be done explicitly with
\code{\longarg{packages} mfirstuc}. Words to be excluded from the
...
can use \longarg{custom-packages} to load a simple package that contains the
required \cs{MFUnocap} commands (in a similar style to
...
in \TeX\ or by the \TeX\ parser library, if \sty{mfirstuc} has been
loaded, when interpreting field values).  Only words in the
exclusion list that start with an alphabetical character can be
matched.  Punctuation following a word-boundary is not considered
