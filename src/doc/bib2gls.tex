...
\longarg{group} (or \shortargfmt{g}) switch. If an
entry is copied, you can use this option to identify a field in which to save
...
This option is ignored for entries that are defined
(with \csopt[define]{action} or \csopt[define or copy]{action}) rather than
copied. This option is not used by \csopt{secondary} which will
...
before the \iext{glstex} file has been created.)
...
\item\optfmt{category}: if sort values are identical, compare
the category fields.
\end{itemize}
...
setting will fallback on comparing the labels.
Since all labels must be unique, this means the comparisons
are all either strictly higher or strictly lower (except when
comparing identical entries).
...
\item\optfmt{non-unique}: append a numeric suffix to non-unique
  \field{sort} values;
\item\optfmt{category}: append the entry's category (if set) to
  the \field{sort} field.
\end{itemize}
...
\csopt[id]{identical-sort-action} to prevent problems occurring with
duplicate sort values.

If \csopt{identical-sort-action} isn't set to \optfmt{id},
then there may be duplicate sort values. To prevent
problems occurring with the hierarchy, this option may be
used to append a suffix to sort values.
...
The \csopt[category]{sort-suffix} setting will only append a suffix
if the entry's \field{category} field is set, but (if set) it will
apply the suffix to all \field{sort} values, even those that are
unique.
