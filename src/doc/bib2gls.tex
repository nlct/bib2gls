...
algorithm used by the corresponding \meta{method} comparator.
However \optfmt{\meta{method}\dhyphen reverse} may not produce a
list that's the exact reverse of the underlying non-reversed
\meta{method}. The reverse sort maintains hierarchy if the
underlying \meta{method} supports hierarchical sorting, so
sub-entries will still be listed after the parent entry.
