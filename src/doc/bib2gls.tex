...
\end{verbatim}
then the interpreter will convert this to \code{\char"2248 3.14} but
\begin{codeenv}
\csopt[\marg{\cs{glshex}2248}\marg{}]{sort-replace}
\end{codeenv}
can be used to strip the $\approx$ symbol so that the value can
now be parsed as a number if \csopt[double]{sort} has been used.

Use \csopt{dual-sort-replace} for duals
