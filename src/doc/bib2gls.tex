...
For example, if the first string is \code{abc12foo} and the second
string is \code{abc6bar} then the sort method performs a letter
comparison on the first three letters (\code{abc}). At this point
there's no difference detected, so the comparator moves onto the
next character, \code{1} for the first string and \code{6} for the
second string. At this point the letter comparators above would
consider \code{1} less than \code{6} since the character \code{1}
(0x0031) comes before the character \code{6} (0x0036), but with the
letter-number comparators, these are both recognised as digits and
it reads in the following digits (if present). For the first case,
this is the sub-string \code{12} and, for the second case, \code{6}.
These are both compared according to their integer representation $6
< 12$, so \code{abc6bar} is considered less than \code{abc12foo}.

Signed integers are also recognised, so \code{abc-12foo} is less
than \code{abc+6bar}, which is again different from the result
obtained with a straight letter comparator where \code{+} (0x002B)
comes before \code{-} (0x002D). The sign must be followed by at
least one digit for it to be recognised as a number otherwise it's
treated as a punctuation character.

In the situation where only one sub-string is numeric, for example
comparing \code{abcXYfoo} and \code{abc6bar}, then the comparator
considers numbers to come before lower case letters. The
case-insensitive comparators always convert the \field{sort} value
to lower case before sorting, so with a case-insensitive sort
\code{abcXYfoo} would first be converted to \code{abcxyfoo}, which
means that \code{abc6bar} comes before it. With the case-sensitive
sort, \code{abc6bar} comes after \code{abcXYfoo}.

\begin{itemize}
\item \optfmt{letternumber-case}: case-sensitive
letter-number (character code) sort.
...
comparators, they don't recognise the superscript or subscript
digits.
\begin{itemize}
...
The value may be \optfmt{doc} (use the document's locale)
or a valid language tag identifying the locale.
