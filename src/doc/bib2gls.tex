...
\glsaddstoragekey{note}{}{\entrynote}
...
\newcommand*{\summaryfmt}[1]{\hyperit{#1}}
...
{%
  ...
  \renewcommand*{\glossentry}[2]{%
   ...
   {%
     \glstarget{cs.##1}{\strut}\gls[format=summaryfmt,hyper=false]{##1}%
     \glsxtrifhasfield{useri}{##1}{\glscurrentfieldvalue}{}%
     \nopagebreak\par\hspace{10pt}%
     \Glossentrydesc{##1}%
   }%
...
The default behaviour of \ics{rgls}\margm{label}\oarg{insert} is to check
...
it does \ics{rglsformat}\marg{label}\marg{insert} instead.
If the use of \ics{rglsformat} is triggered in this way,
then \ics{rgls} writes a record to the \iext{aux} file
with the \glsopt{format} set to \ics{glstriggerrecordformat}.
...
\ics{glsentrytext}). You can fetch the value with
\ics{glsxtrusefield}.  (See the \styfmt{glossaries-extra} manual for
...
ignored record.  (This format is used by \ics{glsaddallunused} but
...
with \ics{glshyperlink}. If \meta{boolean} is missing \optfmt{true}
is assumed.
