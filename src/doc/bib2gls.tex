...
(The \exfile{sample-media.tex} file requires at least \styfmt{datatool}
v2.28.)
...
(or \qt{log line}). The \field{author} field could use \BibTeX's
syntax instead with \csopt{bibtex-contributor-fields} to convert it.
...
(as the \field{cast} field does). Some of
...
\field{cast} field only contains the principle actors rather than the
...
is provided the post punctuation may then be required.

An alternative approach is to use
\csopt[check]{post-description-dot}. Unfortunately in this case 
the question mark is hidden inside the \cs{gls} command.  However
this method would put the punctuation before the
\idx{postdescriptionhook}.
...
\ics{DTLlistformatlastsep}:
\begin{verbatim}
\renewcommand*{\DTLlistformatlastsep}{ and }
\end{verbatim}
...
I've provided a \idx{postdescriptionhook} to append the cast list:
\begin{verbatim}
...
\end{verbatim}
This means that I need to strip \ics{nopostdesc} from the
\field{description} fields with \csopt{strip-trailing-nopost},
and I've used the \styopt{postdot} package option to append a full
stop.
