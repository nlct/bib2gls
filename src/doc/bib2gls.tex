...
\texttt{\$\meta{maths}\$}, \cs{ensuremath}\margm{maths} 
...
\begin{enumerate}
\item if \texttt{\$\meta{maths}\$} then stop (no case change);
\item if \cs{NoCaseChange}\margm{text} or
\cs{ensuremath}\margm{maths} or \cs{si}\margm{text} then skip;
...
short={{ht}ml}
\end{verbatim}
would end up as \texttt{HTml} since it detects the grouping. 
...
short={{}html}
\end{verbatim}
then the result is just \texttt{html}. If a command is followed by 
