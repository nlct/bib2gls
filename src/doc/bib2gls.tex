...
Don't automatically set the \field{group} field (default).
...
The \csopt{group} option may only be used with the \longarg{group}
switch.  This will set the \field{group} field to \meta{label}
unless \meta{label} is \optfmt{auto}, in which case the value is set
automatically during the sorting (see also \csopt{group-formation}
and \sectionref{sec:logicaldivisions}).  The corresponding title can
be set with \gls{glsxtrsetgrouptitle} in the document if the title
is different from the label.  The default behaviour is
\csopt[auto]{group}.
...
This file also includes \atentry{index} and \atentry{indexplural}
entries whose labels match the custom \fieldfmt{identifier} fields.
This means that you can, for example, alias \fieldfmt{identifier} to
\field{parent}, as is done in \exfile{sample-textsymbols2.tex}.
...
\end{codeenv}
I want all the symbols but not the \atentry{index} or
\atentry{indexplural} entries, so I've used a combination of
\csopt[all]{selection} (to select all) and \csopt{match} (to filter
out the unwanted entries):
\begin{codeenv}
  \csopt[all]{selection},
  \csopt[entrytype=symbol]{match}
\end{codeenv}
An alternative filter criteria is:
\begin{codeenv}
  \csopt[original entrytype=icon]{match}
\end{codeenv}
(or \csopt{not-match} could be used but would be more complicated in
this case).
...
\csopt[icon=symbol]{entry-type-aliases}
\end{codeenv}
Unlike the previous example, the custom \fieldfmt{identifier} field
is now aliased to \field{parent}. The labels match the labels of the
\atentry{index} and \atentry{indexplural} entries at the start of
\exfile{miscsymbols.bib}. These represent the topics and unlike the
previous example it's now possible to sort by the topic title
(obtained from the \field{name} field) instead of by the label.
\begin{codeenv}
\csopt[
...
by the \field{name}. This is the default fallback if the
\field{sort} field is missing for \atentry{index} and \atentry{indexplural}.
The default fallback for \field{sort} field for \atentry{symbol}
