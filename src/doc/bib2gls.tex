...
The dual entries such as \atentry{dualindexentry} (described in
\sectionref{sec:dualentry}) are designed to provide a way of
including an entry in both a list of terms (or symbols or
abbreviations) optionally with a description and also include the
term (without the description) in an index.  Additional terms that
should only appear in the index can be defined with \atentry{index}.

Although \bibgls\ is designed to create indexes as well as glossary
lists using the same interface (commands like \cs{gls}), it is
possible to have a mixture of \bibgls\ and \cs{index}, although
it's not recommended and adds to the build complexity. However, if
you really want to automatically \cs{index} an entry every time it's
used with commands like \cs{gls}, then you can use the following
approach:
\begin{verbatim}
...
pdflatex myDoc
\end{verbatim}
Note that this will use the \field{sort} value obtained from
\bibgls.
...
if the \iext{aux} file contains \gls{glsxtrresourcefile}. For
...
\item If \gls{@istfilename} is found in the \ext{aux} file then:
