...
\GlsXtrLoadResources[src=terms,type=terms,save-locations=false]
...
\listoftables
\listoffigures
\printunsrtglossary*[type=terms,style=index,groups=false]
{%
...
The \gls{tertiaryentry} will never have any locations. Any records
...
\item if \csopt[only unrecorded parents]{flatten-lonely-rule}
      then the \parent\ can't have a location list,
      where the location list includes records and \field{see} or \field{seealso}
      cross-references (for the other rules the \parent\ 
      may have a location list as long as it only has the one \child\ selected).
\end{itemize}
...
Only the \glspl{parententry} that don't have a location list will be
...
\iglspl{ignoredrecord}) with no range formations. Any explicit range markup 
...
format for the principal records in the \field{location} field is
...
want to lose a partial location match (for example, if the 
...
setting in the above indicates that locations encapsulated with
...
location in the \gls{mainglossary}.
...
Note that in this case, from \bibgls' point of view, the principal format is
...
\field{primarylocations} so that only the principal records are listed. Since
...
they were identified as principal records). These just show the page
...
manually adding locations in \glspl{supplementaldocument} through the use
...
location (S1) from the \supplementarydocument. The manual approached
...
\supplementarydocument\ with the location as the hyperlink text.
...
the original location format from the \supplementarydocument\
...
If an entry has both a main location list and a
...
This setting allows the location lists for each \gls{primaryentry}
...
list. The \glsdisp{primaryentry}{primary's} location list is emptied.
\item\optfmt{primary} Only the \primary\ is given the combined
location list. The \glsdisp{dualentry}{dual's} location list is emptied.
...
have locations. This is actually preferable for this type of
...
These commands deal with the way the location lists and 
...
from a \supplementarydocument\ are encapsulated within the \meta{list}
part of \gls{bibglssupplemental}. The first argument \meta{n}
(ignored by default) is the number of \supplementarylocations.
