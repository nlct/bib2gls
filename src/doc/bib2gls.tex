...
related) \styfmt{glossaries-extra} entry definitions per \bibgls\
...
related to the \optref{secondary} option).
For example
\begin{verbatim}
...
\newglossaryentry{dual.svm}{name={support vector machine},
 description={statistical pattern recognition technique}}
\end{verbatim}
...
Any fields that aren't listed in the mappings are simply copied,
...
abbreviations defined with \atentryref{dualentryabbreviation}
...
behave as \code{@dualentryabbreviation}. You can have more than
one mapping. For example:
\begin{verbatim}
\GlsXtrLoadResources[
  src=entries,% data in entries.bib
  entry-type-aliases={
    % @abbreviation -> @dualentryabbreviation:
    abbreviation=dualentryabbreviation,
    % @entry -> @index:
    entry=index
  }
...
This is like \csopt{dual-entry-map} but applies to
\atentryref{dualentryabbreviation} rather than \atentry{dualentry}. 
Note that the \field{alias} field can't be mapped. The
default setting is:
\begin{verbatim}
dual-entryabbrv-map=
...
This is analogous to \csopt{dual-entry-backlink} but for entries
defined with \atentryref{dualentryabbreviation} instead of
\atentry{dualentry}.
...
\csopt[\meta{boolean}]{dual-indexentry-backlink}, and
...
\begin{definition}
\cs{bibglsnewdualindexabbreviation}\margm{dual-label}\margm{label}\margm{options}\margm{name}\margm{short}\margm{long}\margm{description}
\end{definition}
This command is used to define primary terms identified with the
...
\providecommand{\bibglsnewdualindexabbreviation}[7]{%
  \longnewglossaryentry*{#2}{%
    name={\protect\bibglsuseabbrvfont{#4}{\glscategory{#1}}},%
    category={index},#3}{}%
\end{verbatim}
...
\end{definition}
to format the name. This ensures that it uses the same font as the
short form for the dual abbreviation.
