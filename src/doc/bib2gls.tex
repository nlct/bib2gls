...
\fielddef{dualdescription} & The description for the dual entry
mapped by \atentryref{dualindexentry} and
\atentryref{dualindexsymbol}.\tabularnewline
...
\section{\atentry{dualindexentry}}
\labelatentry{dualindexentry}

There's only one required field: \field{dualdescription}. This is a
field provided by \bibgls. The primary entry behaves like
\atentryref{index} and the dual entry behaves like \atentryref{entry}
(where the \field{description} is obtained from
\field{dualdescription}).  The default field mapping is:
\begin{itemize}
\item \field{dualdescription} $\mapsto$ \field{description}
\end{itemize}
If no \field{name} is given, the dual
entry is assigned the (unprefixed) entry label. For example:
\begin{verbatim}
...
\end{verbatim}
but they are linked so that if one entry is used the other is
automatically selected (if the \optref{selection} setting includes
dependencies).

The primary entries are defined using \csref{bibglsnewdualindexentry}
...
\section{\atentry{dualindexsymbol}}
...
\item \field{dualdescription} $\mapsto$ \field{description}
\end{itemize}
The required fields are: \field{symbol} and \field{dualdescription}.
If the \field{name} field is omitted, the dual entry is assigned a
name from the primary's (unprefixed) label.
...
\end{verbatim}
but the terms are dependent on each other.
...
pdflatex myDoc
\end{verbatim}
After this, you can switch back to just:
\begin{verbatim}
pdflatex myDoc
bib2gls -g myDoc
pdflatex myDoc
\end{verbatim}
until you reference a new entry.
...
This is like \csopt{dual-entry-map} but applies to
\atentryref{dualindexsymbol} rather than \atentry{dualentry}. 
...
This is analogous to \csopt{dual-entry-backlink} but for entries
defined with \atentryref{dualindexsymbol} instead of
\atentry{dualentry}.
