...
The \meta{fallback-field} may be a known field but not an internal
field. It can't be the \field{sort} field. Unlike the options such
as \csopt{entry-sort-fallback}, the field can't be a keyword to
identify the label and can't be a composite.
...
\csopt{entry-type-aliases}). This will override any of the
\optfmt{\meta{entrytype}\dhyphen sort\dhyphen fallback}
...
\begin{codeenv}
\csopt[abbreviation=long,acronym=short]{custom-sort-fallbacks} 
\end{codeenv}
...
\begin{codeenv}
\csopt[abbreviation=long]{custom-sort-fallbacks} 
\end{codeenv}
...
\begin{codeenv}
\csopt[dualnumber=user1,dualsymbol=user2]{custom-sort-fallbacks}
\end{codeenv}
...
may use \ics{u}\meta{hex} to indicate a character by its hexadecimal
code. For example:
\begin{codeenv}
\gls{GlsXtrLoadResources}\oarg{
  \csopt[long+short]{abbreviation-sort-fallback},
  \csopt[]{field-concat-sep}
}
\end{codeenv}
This will obtain the sort value for abbreviations from a
concatenation of the \meta{long} and \meta{short} values without a
separator.
...
string. You can instead do \csopt[\stringu32]{field-concat-sep} but
