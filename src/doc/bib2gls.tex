...
{%
  ...
  \renewcommand*{\glossentry}[2]{%
   ...
   \hypertarget{topic.##1}{}%
   \pdfbookmark[1]{\thisvalue}{topic.##1}%
   \section*{\glsdisp{##1}{\thisvalue}}%
...
%
     ...
     {%
        ...
        {%
          ...
          {%
            ...
            {\space(\emph{see} \hyperlink{topic.\glscurrentfieldvalue}%
             {\glsentryname{\glscurrentfieldvalue}})}{}%
          }%
   ...
   {%
     ...
     \pdfbookmark[2]{\thisvalue}{topic.##2}%
     \subsection*{\glsdisp{##2}{\thisvalue}}%
...
\newcommand*{\printcommandtopic}{%
  \printunsrtglossary*[type=index,
    style=commandtopic,
    title={Summary by Topic of Commands Provided by 
     \styfmt{glossaries}, \styfmt{glossaries-extra} and
     Supplementary Glossary Packages},
    toctitle={Summary by Topic},
    label={sec:topics},
    nogroupskip,
    nonumberlist
  ]%
...
one argument. During the processing stage, each field identified in
...
arguments. During the processing stage, each field identified in the
...
their interpreted values. The value should be a comma-separated list
of field names. If the list is empty, this action is switched off
(default). Other fields not listed may still be interpreted
...
\atentry{symbol}\marg{sigma,
  \field{name} = \marg{\cs{ensuremath}\marg{\cs{sigma}}},
...
\cs{newcommand}\marg{\cmd{definition}}[1]\marg{\comment{}
  ...
  \marg{\comment{}
    \cs{section}\oarg{\cs{glsentryname}\marg{\idx{param}}}\marg{\cs{glsadd}\marg{\idx{param}1}\cs{glsxtrglossentry}\marg{\idx{param}1}}\comment{}
    \cs{Glossentrydesc}\marg{\idx{param}1}\cs{glspostdescription}
  }\comment{}
  {\cs{section}\oarg{Missing `\idx{param}1'}\marg{\cs{glsadd}\marg{\idx{param}1}}}\comment{}
}
\end{codeenv}
...
\gls{GlsXtrLoadResources}\oarg{
  \csopt[entries]{src},
  \csopt[\field{name}=\field{user1}]{replicate-fields},
  \csopt[fallback]{replicate-missing-field-action},
  \csopt[\field{user1}]{interpret-fields}
}
\end{codeenv}
This first copies the \field{name} field to one of the custom fields
(\field{user1}) and then interprets the custom field. This leaves
the \field{name} field with the \LaTeX\ code to produce the symbol
in the document, but the \field{user1} field ends up with all markup
...
bookmarks by adjusting the definition of my custom command:
\begin{codeenv}
\cs{newcommand}\marg{\cmd{definition}}[1]\marg{\comment{}
  \cs{ifglsentryexists}\marg{\idx{param}1}\comment{}
  \marg{\comment{}
    \cs{section}
    \oarg{\cs{texorpdfstring}\marg{\cs{glsentryname}\marg{\idx{param}1}}\marg{\cs{glsentryuseri}\marg{\idx{param}1}}}
...
field that needs a case-change applied. The \meta{setting} should be
the same as the permitted values for the above options. This
option is applied after all fields have been parsed. If the
