...
\atentry{bibtexentry} then they can be made dependent entries and
the \field{bibtexentry} internal field can be set for the
...
\atentry{contributor} entries and vice versa. Each contributor is effectively
defined as
\begin{codeenv}
...
\end{codeenv}
for each \atentry{bibtexentry} associated with that contributor. The
...
For example:
\begin{verbatim}
\newcommand{\contributorhandler}[1]{\par\glsentryname{#1}}
...
\end{verbatim}
(where the resource option \csopt[\marg{title=name}]{field-aliases}
has been used).
