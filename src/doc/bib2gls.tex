...
trailing spaces internally trimmed by the \sty{xkeyval} package, so
...
\meta{bib file} already exists with the default \longargfmt{overwrite}. 
...
\item[\longargfmt{space-sub} \meta{replacement} or \shortargfmt{s} \meta{replacement}] 
...
\item[\longargfmt{ignore-type}] Ignore the \field{type} field.
You may find it more flexible not to be locked into a specific
glossary type if you have a large database of entries.

\item[\longargfmt{no-ignore-type}] Don't ignore the \field{type} field
(default unless \longargfmt{split-on-type}).

\item[\longargfmt{split-on-type} or \shortargfmt{t}] 
Split the entries into separate files according to the \field{type}
field. Any entries that don't have the \field{type} field set or
have it set to \ics{glsdefaulttype} will be written to \meta{bib
file}. Otherwise entries will be written to
\metafilefmt{}{type}{.bib} (in the same directory as \meta{bib
file}) where \meta{type} is the value of the \field{type} field.
This option automatically implements \longargfmt{ignore-type} and
\longargfmt{no-overwrite}.

\item[\longargfmt{no-split-on-type}] Don't split the entries into separate
files (default).

\item[\longargfmt{preamble-only} or \shortargfmt{p}] Only parse the document preamble.

\item[\longargfmt{no-preamble-only}] Parse the entire document
(default).

\item[\longargfmt{locale} \meta{lang tag}] Identifies the \langxml\
to use for \idx{convertgls2bib}['s] messages.

\item[\longargfmt{overwrite}] Allow existing \ext{bib} files to be
overwritten. (Default unless \longargfmt{split-on-type}.)

\item[\longargfmt{no-overwrite}] Don't allow existing \ext{bib} files to be
overwritten. (Default if \longargfmt{split-on-type}.)
