...
class so there should be no issues with spaces in the argument. 
The \longarg{debug} switch will write the process call in the
transcript file and will delimit the argument in the log with single
quote characters for convenience.

\bibgls\ creates files with the extension \ext{glstex}, which are
input by \gls{GlsXtrLoadResources}. This extension is fixed and is
imposed by both \bibgls\ and \gls{GlsXtrLoadResources}. \bibgls\ also
creates a transcript file with the default extension \ext{glg}. This
may be overridden by the \longarg{log-file} switch, but \bibgls\
always forbids write access to any file with the following
extensions: \extfmt{tex}, \extfmt{ltx}, \extfmt{sty}, \extfmt{cls},
...
will always be empty. The most reliable target is \meta{hcounter},
...
\cs{newcommand}\marg{\gls{bibglsnewbibtexentry}}[4]\marg{\%
...
loads the supplementary \isty{glossaries-extra-bib2gls} package
...
For convenience, the \sty{glossaries-extra-bib2gls} package v1.37+
provides \ics{glsxtrSetWidest}, which will set the widest name for 
those styles that need it. As from version 1.8, \bibgls\ now checks
for the existence of this command and will use it with
...
redefine this:
\begin{codeenv}
\cs{renewcommand}*\marg{\cs{glsxtrbookindexprelocation}}[1]\marg{\idx{commentchar}
  \cs{glsxtrifhasfield}\marg{primarylocations}\marg{\idx{param}1}\idx{commentchar}
  \marg{\idx{commentchar}
    \ics{glsxtrprelocation}
    \cs{glscurrentfieldvalue}
    \cs{glsxtrifhasfield}\marg{location}\marg{\idx{param}1}\marg{;}\marg{}\idx{commentchar}
  }\idx{commentchar}
  \marg{}\idx{commentchar}
...
\begin{codeenv}
\cs{printunsrtglossary*}\marg{\idx{commentchar}
  \cs{renewcommand}\marg{\cs{GlsXtrLocationField}}\marg{primarylocations}\idx{commentchar}
}
\end{codeenv}
...
\ics{printunsrtglossary*}\oarg{style=tree,nonumberlist}\marg{\idx{commentchar}
 \cs{renewcommand}*\marg{\ics{glsextrapostnamehook}}[1]\marg{\ics{glsadd}\oarg{format=hyperemph}\marg{\idx{param}1}}\idx{commentchar}
}
...
\ics{printunsrtglossary*}\oarg{style=tree,title={Summary}}\marg{\idx{commentchar}
 \cs{renewcommand}*\marg{\ics{glsextrapostnamehook}}[1]\marg{\ics{glsadd}\oarg{format=hyperemph}\marg{\idx{param}1}}\idx{commentchar}
 \cs{renewcommand}\marg{\ics{GlsXtrLocationField}}\marg{primarylocations}\idx{commentchar}
}
...
\begin{codeenv}
\cs{newcommand}\marg{\gls{bibglsnewbibtexentry}}[4]\marg{\idx{commentchar}
