...
This setting is always implemented after \csopt{field-aliases}. If a
field has been aliased then the original field name is no longer
present and so ignoring it will have no effect.
...
where all values are known field names. The value is required for this key
but may be empty, which indicates that the setting is switched off.

\begin{important}
Note that internal fields such as \field{originalid} aren't actually
set as a field within \bibgls\ but are simply written to the
\ext{glstex} file after the resource set has been processed.
Therefore they can't be used as the source \meta{field1}.
\end{important}
...
If the target field doesn't have an associated key
recognised by \gls{newglossaryentry} then the value will be saved
using \cs{GlsXtrSetField}. Avoid using special internal fields as
