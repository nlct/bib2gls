...
and \ics{@currentHref}. Instead of writing the record as:
\begin{codeenv}
...
will always be empty. If either \meta{title} or \meta{href} is
empty, \bibgls\ will treat the record as though it had been
identified with the normal \cs{glsxtr@record}.
...
The \meta{title} will also be empty at the
start of the document. With \sty{hyperref}, the \meta{href} will be
...
results. Since numbered equations don't naturally have a title, the
\sty{glossaries-extra} package switches to normal records in
known numbered maths environments.

For compactness, \bibgls\ tries to merge duplicate or near
duplicate records. There are three possible rules that it will
...
and \meta{href} parts.
\end{itemize}
The default \meta{rule} is \optfmt{location}. Note that for all
...
(This option can conflict with the \styopt[nameref]{record}
option, so it's best not to use them both.)
