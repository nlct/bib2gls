...
file, so it doesn't affect selection criteria. The actual 
suffix is formed from \meta{value}\meta{n} where \meta{n} is
an integer that's incremented in the event of multiple duplicates.
For example, \csopt[.copy]{duplicate-label-suffix} will 
change the label to \meta{id}\code{.copy1} for the first duplicate of
the entry whose label is \meta{id}, 
\meta{id}\code{.copy2} for the second duplicate, etc.
...
\gls{GlsXtrLoadResources}\oarg{
 ...
 \csopt[baseunit]{type},
 \csopt[rep.]{label-prefix},
 \csopt[rep.]{record-label-prefix},
 \csopt{set-widest},
 \csopt[name]{symbol-sort-fallback},
 \csopt[false]{save-locations}
}
...
\ics{glsxtrnewgls}\marg{rep.}\marg{\csfmt{baseunit}}
