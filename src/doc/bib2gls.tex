...
The \csopt{sort} key indicates how entries should be sorted. Allowed
values are listed below.
The first set of possible values don't actually perform any sorting.
\begin{itemize}
...
This next set of possible \optfmt{sort} values use rule-based comparators, which
usually ignore most punctuation and may ignore modifiers (such as
accents). Use with \optref{break-at} to determine whether or not to split at word
boundaries.
\begin{itemize}
...
The following use letter case comparators. These simply compare the
character codes. The case-insensitive options first convert the
...
\item \optfmt{letter-nocase}: case-insensitive letter sort.

\item \optfmt{letter-nocase-reverse}: reverse case-insensitive letter sort.
\end{itemize}

The following use a letter-integer hybrid. They behave much like the
...
identical, so the comparator moves on to the next character of each
string. These are now both recognised as digits, so the comparator
...
(that is, \code{abc12bar} comes after \code{abc6foo}).  The same
result occurs for other numbering systems,  for example if the
Basic Latin digits 1, 2 and 6 are replaced with the corresponding
Devanagari digits. (But note that the non-digit character comparisons will
still be based on their Unicode values not according to a particular
locale. This type of sort method is intended primarily for symbolic values,
...
\optref{letter-number-rule} (for primary entries) is used to
determine the result. Dual and \optref{secondary} sorting
are governed by the corresponding \optref{dual-letter-number-rule}, and 
\optref{secondary-letter-number-rule}.

For example, if comparing \code{abcXYfoo} and \code{abc6bar} with
\csopt[letternumber-case]{sort} or \csopt[letternumber-nocase]{sort}, 
then the default \optref[before lower]{letter-number-rule} setting
considers numbers to come before lower case letters. The
case-insensitive comparators always convert the \field{sort} value
to lower case before sorting, so with a case-insensitive 
\csopt[letternumber-nocase]{sort} \code{abcXYfoo} would first be
converted to \code{abcxyfoo}, which means that \code{abc6bar} comes
before it. With the case-sensitive \csopt[letternumber-case]{sort}, 
\code{abc6bar} comes after \code{abcXYfoo}. 
...
The following use numeric comparisons. The sort value is expected to
...
\end{itemize}
The following values are for dates and times. Use \optref{date-sort-format}
...
the date is in the format particular to your locale. In which
...
$a$ is an integer representation of the era ($-1$ for \era{BC}).
...
\begin{verbatim}
20170110 -> '+1 2017-01-10' [20170110]
\end{verbatim}
The first value is the label (\code{20170110}), the second
...
\begin{verbatim}
Warning: Can't parse sort value '10 Jan 2017' for '20170110'
(pattern: 'dd/MM/yyyy')
\end{verbatim}
This shows the field value that \bibgls\ is trying to
parse (\code{10 Jan 2017}) for the entry identified by
the given label (\code{20170110}). The pattern \bibgls\ 
...
If you use \optref[numberformat]{sort} or
...
For dual or \optref{secondary} numeric sorting, use
\optref{dual-numeric-sort-pattern} and
\optref{secondary-numeric-sort-pattern},
respectively.
...
\end{itemize}
For dual or \optref{secondary} numeric sorting, use
\optref{dual-numeric-locale} and \optref{secondary-numeric-locale},
respectively.
