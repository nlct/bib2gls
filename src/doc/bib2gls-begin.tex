...
converts the first letter of each \field{name} to upper case in the
...
formatting, but it's also useful to have a place in the document
...
\sectionref{sec:multigloss}) in the order of definition.
...
with the \sty{glossaries-extra-stylemods} package). For example, the
...
duck and goose above). The letter group is determined by the first
letter of the \field{sort} field. Since the default behaviour of
both \sty{glossaries} and \sty{glossaries-extra} is to use
\idx{makeindex}, the \field{sort} field (which is used by
\idx{makeindex}) is set to the value of the \field{name} field
(unless explicitly set) and then sanitized.
...
instead. The value of this field must be a label (see
\sectionref{sec:labels}). A title may be
...
\section{Homographs and Hierarchical Terms}
\label{sec:parent}

An entry may be assigned a parent with the \field{parent} key. The
value must be the label of a entry that's already defined. If the
\field{name} key is omitted, the value is assumed to be the same as
the parent's \field{name}. For example:
\begin{codeenv}
\comment{parent:}
...
The \glostyle{treenoname} or \glostyle{treenoname} styles are
...
This produces
\begin{result}
\glsaddeach{hom.glossary,hom.glossarycol,hom.glossarylist}%
\setupglossaries{toc=false,section}%
\glossariesextrasetup{postpunc=dot}%
\renewcommand*{\printunsrtglossaryentryprocesshook}[1]{%
...
caused an odd effect with a space occurring between the name and 
the post-description punctuation. This can be removed by redefining
\nosecformatdef{glstreenonamedesc}
so that it checks if the \field{description} field has been set
with:
...
\cmd{renewcommand}\marg{\gls{glstreenonamedesc}}[1]\marg{\comment{}
  ...
  \marg{}\comment{description field is empty}
}
\end{codeenv}
...
\sectionref{sec:symbols}) which sets the glossary to
...
There's no optional argument for this command.
...
\sectionref{sec:postfieldhooks}), and the \catattr{glossname}
and \catattr{glossnamefont} attributes (see \sectionref{sec:glossname}).
You then need to add the other field values as appropriate.
To pick up the \styopt{postpunc} setting and the
\idx{postdescriptionhook}, append
...
\gls{glsxtrglossentry}\marg{area} \gls[noindex=false]{colsep} \gls{glsentrydesc}\marg{area}\gls{glspostdescription}\gls[noindex=false]{cs.backslash}
\gls{glsxtrglossentry}\marg{pi} \gls{colsep} \gls{glsentrydesc}\marg{pi}\gls{glspostdescription}\gls{cs.backslash}
\gls{glsxtrglossentry}\marg{radius} \gls{colsep} \gls{glsentrydesc}\marg{radius}\gls{glspostdescription}
...
\end{equation}
\begin{tabular}{ll}
\glsxtrglossentry{standalone.area} & \glsentrydesc{standalone.area}\glspostdescription\\
\glsxtrglossentry{standalone.pi} & \glsentrydesc{standalone.pi}\glspostdescription\\
\glsxtrglossentry{standalone.radius} & \glsentrydesc{standalone.radius}\glspostdescription
\end{tabular}
\end{result}
The purple text shows the hyperlinks to the relevant definition. The
hypertargets are prefixed with
\nosecformatdef{glolinkprefix}
This can be locally changed to avoid clashes if the definition needs
...
\idx{firstuseflag} is unset. This is intended for use where the
switch causes a problem, but it can also be used in this case to
store a list of used entries (since there's no difference between
\idx{firstuse} and subsequent use in this case, it won't affect the
\idx{link-text}). The buffering is started with:
...
\cmd{newcommand}\marg{\cmd{doglossaryentry}}[1]\marg{\comment{handler macro}
  \gls{glsxtrglossentry}\marg{\gls{param}1} \gls{colsep} \gls{glsentrydesc}\marg{\gls{param}1}\gls{glspostdescription}\gls{cs.backslash}\comment{}
}
\strut
...
Instead of using \gls{glsentrydesc}, you may find it more convenient
to use:
\nosecformatdef{glossentrydesc}
Unlike
\gls{glsentrydesc}, which just displays the value of the
...
\nosecformatdef{Glossentrydesc}
which converts the first letter of the description to uppercase.
...
command, but it does support the \styopt{accsupp} package option. 
...
letter of the \field{name} to uppercase. Unfortunately this can't be
...
an uppercase character and the \field{text} field in lowercase. For
example:
\begin{codeenv}
\gls{newglossaryentry}\marg{amethyst}
\marg{
...
\code{firstuc} (convert the first letter to uppercase), \code{title}
...
\code{firstuc} (convert the first letter to uppercase) and
...
\gls{glspostdescription}. The \sty{glossaries-extra-stylemods}
