...
automatically to the location list, so I've used the
...
written to the \ext{glstex} file. This group label is recognised by
...
in English). So all the glossary will start with a symbols group
...
is analogous to:
\begin{codeenv}
\gls{newglossaryentry}\marg{duck}\marg{\field{name}=\marg{duck},\field{description}=\marg{}}
\end{codeenv}
and
\begin{codeenv}
...
\atentry{index}\marg{chateau,
  \field{name} = \marg{ch\^ateu},
  \field{plural} = \marg{ch\^ateux}
}
\end{codeenv}
is analogous to:
\begin{codeenv}
...
\marg{
 \field{name}=\marg{ch\^ateau},
 \field{plural}=\marg{ch\^ateaux},
 \field{description}=\marg{}
}
\end{codeenv}
