...
point in the document, but may also be copied to the table of
contents or list of figures etc. Additionally, depending on the page
style, the section argument may also be reproduced in the page
header. This repeated use of the same material can cause
complications, in particular it can prematurely triggering the 
\idx{firstuseflag} switch and cause unwanted indexing. If the
content appears in the page header and the page styles converts
headers to upper case, this can also cause a problem.
...
a multi-paged document as the problem doesn't occur until the second
...
Here's an example that doesn't cause an error, but does cause
unexpected output:
\begin{codeenv}
\cmd{documentclass}\marg{book}
\strut
...
appears as \qt{A chapter about hypertext markup language (HTML)},
which shows the \idx{firstuse} of \code{\gls{gls}\marg{html}}. The
\ext{toc} file (which was created but not read by
\gls{tableofcontents}) now contains:
\begin{codeenv}
...
but the chapter title is now \qt{A chapter about HTML}, which shows
the subsequent use.
