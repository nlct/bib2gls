...
\section{SI Units}
\Idxpl{baseunit}: \baseunit{ampere} (measures \measurement{ampere}), 
\baseunit{kilogram} (measures \measurement{kilogram}), \baseunit{metre}, 
\baseunit{second}, \baseunit{kelvin}, \baseunit{mole}, \baseunit{candela}.
...
\Idxpl{film}: \gls{film.anunexpectedjourney}, \gls{film.desolationofsmaug}
and \gls{film.thebattleoffivearmies} (adapted from the
\idx{book} \gls{thehobbit} by \gls{tolkien}), 
\gls{film.thefellowshipofthering}, \gls{film.thetwotowers}
and \gls{film.thereturnoftheking} (adapted from the
\idx{book} \gls{thelordoftherings} by \gls{tolkien}).
